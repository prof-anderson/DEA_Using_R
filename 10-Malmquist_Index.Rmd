...
mdata_19_20_pre <- deaR::make_malmquist(mdata_19_20,
                                percol = 2, 
                                arrangement = "vertical",
                                inputs = 3, 
                                outputs = 4:8)
...
da_f <- data.frame(x= c(11, 29, 31, 61, 13, 27, 17, 61), 
                   y=c(6, 8, 11, 16, 7, 9, 10, 16),
                   d= c(1,2,3,4, 1,2,3,4), 
                   p=c(1,1,1,1,2,2,2,2))

mpi_r <- MPI(Dataset = da_f, DMU_colName = "d", 
...
kbl(head( da_bb19_20), caption="Example of Batting Data")

mpi_bb <- MPI(Dataset = da_bb19_20, DMU_colName = "Player", 
             IP_colNames = "PA", 
...
colnames(res_bb) <- c("E19-T19", "E19-T20", 
                      "E20-T20", "E20-T19", "TEC", "TC")

rownames(res_bb)<-mpi_bb$DMU

kbl(head(res_bb, 20), digits = 4)
...
Let's shift attention to talk about specific players.

```{r}
# Now let's grab just a few rows of specific players to discuss

kbl(res_bb[c("cobbty01", "collied01", "ruthba01"),], digits=4)
```

Ty Cobb's values are interesting.  E19-T19 is one and therefore he was efficient in 1919 using his 1919 batting statistics. He is no longer efficient in 1920 as his as denoted by E20-T20 being 0.9165. The TEC 
