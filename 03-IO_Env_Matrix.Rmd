...
In the previous chapter we created the \index{Data envelopment analysis!linear program} DEA linear program algebraically using the `ompr`package. Before `ompr`was available, we needed to use another approach for most \index{Data envelopment analysis!packages} DEA packages such as \index{TFDEA!package} `TFDEA` and \index{MultiplierDEA!package} `MultiplierDEA`, which consisted of using matrices. This approach requires more "bookkeeping". Both approaches work but I think the algebraic approach is likely to be easier to write, debug, and extend in the future. For solving, we will use the widely adopted \index{lpsolveapi} `lpsolveapi` package but this same matrix-based approach holds other LP solver engines such as C-Plex, Gurobi, and CBC.
...
In this formulation, *X* is a vector of variables. This is a little confusing for people doing DEA since in \index{Data envelopment analysis!inputs} DEA *x* represents inputs. The vector *X* has length equal to the number of units examined plus one more for theta and the number of inputs and the number of outputs to cover slack variables. *C* is a vector of data with the same number of elements as the *X* vector. *A* is a matrix of data and *B* is a vector describing the \index{Constraints} constraint right hand sides.
...
The next step is to incorporate the data into the model. Since there is only one input and one output, there are only two \index{Constraints} constraints and two slack variables. To keep a close connection to the application, we will refer to the specific envelopment variables by DMU name rather than number. For example, this lists $\lambda_A$ rather than $\lambda_1$.
...
The *A* matrix contains the variable's coefficients from the \index{Constraints} constraints.
...
Lastly, the *B* vector contains the right hand sides of the \index{Constraints} constraints.
...
The bulk of the LP is then defined by passing along the numerical A matrix and the vectors of B and C. We then just need to indicate the sense of the the objective function (min or max) and the directions of the \index{Constraints} constraints.
...
Now it it gets more involved. We are defining a function for calculating \index{Data envelopment analysis!returns to scale} DEA with options for returns to scale, \index{Data envelopment analysis!orientation} orientation, and two different options for \index{Data envelopment analysis!super-efficiency} super-efficiency.
...
This is where we start creating the populating the linear programming model. This model declaration creates a problem with the needed number of \index{Data envelopment analysis!variables} columns (variables) for standard DEA envelopment models.
...
The name `lpe` is meant to indicate a linear programming object for the envelopment model. There are two basic DEA models: the \index{Data envelopment analysis!envelopment model} envelopment model and the \index{Data envelopment analysis!multiplier model} multiplier model. They generate the same results but are structured very differently. We'll return to the multiplier model at a later time.

We easily display what our empty `lpe` object looks like. We are initializing it without any \index{Constraints} constraints but columns for each variable that we expect to use:
...
The following step is more complicated but the core idea is to populate a data matrix to describe the \index{Constraints} constraints for a DEA model. The first `for` loop create the row(s) corresponding to input constraints.
...
Next we get input dual weights and place them in `results.vweight`. These are useful because the dual weights are essentially the solution the \index{Data envelopment analysis!multiplier model} DEA multiplier model. We can also thing of them roughly as the "prices" or "values" placed on inputs and outputs that make the unit appear as good as possible. (It is tricky that all rows are offset from how you might count them because row 1 is considered to be the objective function.)

The fourth item gets the dual weight from the returns to scale \index{Constraints} constraint. Notice that this is extracted from row 2 of the dual solution.
...
Now we need to do a step that is called slack maximization. We will need to add a \index{Constraints} constraint that the efficiency variable ($\theta$) is held fixed to the value found in Phase 1.
