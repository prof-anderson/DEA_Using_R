...
## Transitioning to a Matrix Model

In the previous chapter we created the DEA linear program algebraically using the `ompr` package [@SchumacherOmpr2023]. Before `ompr` was available, most DEA packages such as \index{TFDEA} `TFDEA` and \index{MultiplierDEA} `MultiplierDEA`, used a matrix-based approach. This approach requires more "bookkeeping". Both approaches work but I believe the algebraic approach is likely to be easier to write, debug, and extend in the future. For solving, we will use the widely adopted \index{lpSolveAPI} `lpSolveAPI` package, but this matrix-based approach can be implemented using other LP solver engines such as C-Plex, Gurobi, and CBC.

To get  ready for matrix formulating, we need to review the standard form of linear programs which means that only numbers can be on the right hand side of the inequalities.
...
The first step is to simply move the expression with a variable from the right hand side of the input inequality to the left-hand side.
...
Notice that this formulation now includes seven \index{Decision variables} decision variables. The linear programming convention is to define the vector of all variables to be *X*. In this case, $X=[\lambda_A, \lambda_B, \lambda_C, \lambda_D, \theta, s^x_1, s^y_1 ]^\tau$. The $\tau$ is a transpose operator that changes a vector from a column to a row vector or vice versa (row to column). In this case, the *X* is a column vector but the elements are shown as a horizontal row vector to save space and then flipped using the transpose operator. Recall that in R the transpose function is just simply `t()`.
...
## Implementing with `lpSove`

As models get larger, the full numerical representation becomes difficult for humans to read even if it is a very good format for computers. If you stare at that too long, you may go "bug-eyed." Personally, I prefer the algebraic representations. The \index{lpSolve} `lpSolve`package in R is capable of taking an LP model in this form and solving it. Now we just need to structure the data in R to put it in the same format.
...
The solver in this case knew nothing of our model's structure so the results in terms of decision variable values needed to be organized based on how we structured the model to pass into \index{lpSolve} `lpSolve`.
...
The plain text version of the table is functional but can be much better formatted as discussed in Chapter 2.  
...
## Creating the LP Incrementally with `lpSolveAPI`
...
For this walkthrough, we will focus on running the LP to calculate DEA for only one unit. We typically would do a loop for every unit but the entire `for` loop would need to be inside a single R chunk of this document. Instead, we will walk through how the data structure works. If you understand this, extending it to other units with a `for` loop is straightforward.
...
```{r}
lpcontrol <- lp.control(lpe, sense="min") # Set to minimize 
```

The above commands have two important functions. The first merely sets it to be a *minimize* objective function.
...
The next command says what to minimize by placing appropriate 0's and a 1 in their correct place.  We use the `rep` command to repeat the zero a certain number of times. We only want to minimize $\theta$ so we put 0 for each of the $\lambda$ variables. There are `ND` or $N^D$ of the these variables before we get to the $\theta$. We then put a 1 for the $\theta$ variable. We will also use zero for each of the $N^X+N^Y$ slack variables.
...
Let's look at what our current linear program.

```{r Display-lp-with-Obj}
lpe 
```

We have seven separate variables (or columns) for each $\lambda$, one for $\theta$ and the then slacks. They are all continuous variables with lower bounds of zero.  The `Minimize` row shows the coefficients for each variable is zero except for the $\theta$ which is one.  

Now let's add our constraint(s) and display the resulting LP model. The following step is more complicated but the core idea is to populate a data matrix to describe the input constraints for a DEA model. The first `for` loop create the row(s) corresponding to each input constraint. In our example, we only have one input ($N^X=1) but this would work for multiple inputs.


```{r}
for (i in 1:NX) {                  # Repeat for each input
...
Alas, this constraint can be confusing, particularly as models get bigger. Let's better label the rows and columns.

```{r Prepare_names_to_LP}
...
dimnames(lpe) <- list(RowNames, ColNames)  
   # Add Constraint names to LP

lpe 
```

The `Con_X_1` row has the the appropriate values for each of decision variables set that are then added up and set equal to zero. Note that slacks are included here rather than using an inequality constraint.

Now we are ready to move on to the output rows (constraints). \index{Constraints!Rows} (constraints).
...
If the variables had not defaulted to be non-negative, we could Set the lower limits for all variables to be 0.0

```{r, eval=FALSE}
set.bounds (lpe, lower = rep(0.0, ND+1+NX+NY))  
```

Technically the radial efficiency scores, $\theta$ is usually declared to be un-restricted in sign but this won't affect the solution.
...
Simply solving doesn't tell us anything very useful and might even be misinterpreted as the objective function value ($\theta$) being zero. It is actually just returning an error code of 0 (or at least we hope to get that-anything else is worth digging into!) An error code of 0 means that it solved the linear program, `lpe` successfully. When it does so, it adds results to the `lpe` object. Now we need to extract the results from the solution in a form that we can use.
...
results.lambda[k,] <- get.variables(lpe)[1:(ND)] 
    # Put lambda values in k'th row of matrix            
...
Just as we extended the radial DEA model to account for slack maximization, we can do the same thing in the matrix approach. 
...
The following step sets *Phase1* equal to the same efficiency score. We will use this value in a second LP that we refer to as *Phase2* in order to make sure that we have an optimal target. This is consistent with the two-phase approach discussed in Chapter 2.
...
1.  Graphically determine the efficiency score and lambdas for the other units. ![](images/coffee-cup.png){width="2.5%"}
2.  Interpret the solution in terms of who is doing well, who is doing poorly, and who should be learning from whom. ![](images/coffee-cup.png){width="2.5%"}
3.  Add a fifth unit, E, that produces 400 units output using 30 units of input. Graphically evaluate all five units for their efficiency scores and lambda values. ![](images/coffee-cup.png){width="2.5%"}
4.  Interpret the solution in terms of who is doing well, who is doing poorly, and who should be learning from whom. ![](images/coffee-cup.png){width="2.5%"}
5.  Examine another unit using the R code (hint: change where k is set.) ![](images/coffee-cup.png){width="2.5%"}
6.  Interpret the solution in terms of who is doing well, who is doing poorly, and who should be learning from whom. ![](images/coffee-cup.png){width="2.5%"}
7.  Wrap a `for` loop around the model to examine every unit. ![](images/coffee-cup.png){width="2.5%"}
8.  Use a bigger data set (more inputs, outputs, and units.) ![](images/coffee-cup.png){width="2.5%"}
9.  Validate results against other DEA packages (ex. `Benchmarking`, `nonparaeff`) ![](images/coffee-pot.png){width="2.5%"}
10. Construct an example where Phase 2 increases positive slacks from Phase 1. ![](images/coffee-pot.png){width="2.5%"}
11. Create "cool" graphs or plots of results. ![](images/coffee-pot.png){width="2.5%"}
