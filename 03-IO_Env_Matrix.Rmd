...
In the previous chapter we created the DEA linear program algebraically using the new _ompr_ package.  Before _ompr_  was available, we needed to use another approach for our DEA package, _TFDEA_, which consisted of using matrices. This approach  requires more "bookkeeping". Both approaches work but I think the algebraic approach is likely to be easier to write, debug, and extend in the future.  For solving, we will use the widely adopted _lpsolveapi_ package but this same matrix-based approach holds other LP solver engines such as C-Plex, Gurobi, and CBC.   

People interested in working on the TFDEA package should read this chapter carefully.  We will be returning to these topics algebraically as well later since it is conceptually similar to the structure of the package.  For people that want to stick to the algebraic implementation and using _ompr_, you can skip this chapter.  
...
Note that in this form, _X_ is a vector of variables.  This is a little confusing for people doing DEA since in DEA _x_ represents inputs.  The vector _X_ has length equal to the number of units examined plus one more for theta and the number of inputs and the number of outputs.  _C_ is a vector of data with the same number of elements as the _X_ vector.  _A_ is a matrix of data and _B_ is a vector describing the contstraint right hand sides.  
...
Notice that this formulation now includes seven decision variables.  The linear programming convention is to define the vector of all variables to be _X_.  In this case, $X=[\lambda_A, \lambda_B, \lambda_C, \lambda_D, \theta, s^x_1, s^y_1 ]^\tau$.  Notice that _X_ is a column vector transposed to be shown as a horizontal row vector.  Using the _X_ vector for decision variables allows us to separate the data from the variables into 
...
Now, let's show the data elements.  The _C_ vector contains the coefficients of the objective function.
...
The _A_ matrix contains the variable's coefficients from the constraints.
...
Lastly, the _B_ vector constains the right hand sides of the constraints.
...
Now we are ready to start implementing our linear program for DEA in R.
...
* Columns for each lambda, _i.e._ $\lambda_1$, $\lambda_2$, ..., $lambda_{N^D}$  
...
```{r Add_names_to_LP}
    ...
    lambda_names <- paste("L", 1:ND, sep = "")  # We'll use L to represent Lambda
...
The following step sets _Phase1_ equal to the same efficiency score.  We will use this value in a second LP that we creatively refer to as _Phase2_ in order to make sure that we have an optimal target.  This is consistent with the two-phase approach discussed in Chapter 2.
...
#    results.xslack[k,]
#    results.yslack[k,]
    pander(cbind(t(results.lambda[k,]),results.xslack[k,],results.yslack[k,]), 
