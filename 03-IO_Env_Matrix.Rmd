...
In the previous chapter we created the DEA linear program algebraically using the `ompr` package. Before `ompr` was available, most DEA packages such as \index{TFDEA} `TFDEA` and \index{MultiplierDEA} `MultiplierDEA`, used a matrix-based approach. This approach requires more "bookkeeping". Both approaches work but I believe the algebraic approach is likely to be easier to write, debug, and extend in the future. For solving, we will use the widely adopted \index{lpSolveAPI} `lpSolveAPI` package, but this matrix-based approach can be implemented using other LP solver engines such as C-Plex, Gurobi, and CBC.
...
Let's demonstrate the use of \index{LaTeX} LaTeX in table headings. There are several things to bear in mind when using LaTeX rendered in column labels.
