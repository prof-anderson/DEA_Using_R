...
```{r}
rho_numerator <- 1 - rowMeans(Slack1.xslack/x)
rho_denominator <- 1 + rowMeans (Slack1.yslack/y)
Slack1.rho <- rho_numerator/rho_denominator
```
...
rho_numerator <- 1 - rowMeans(Slack2.xslack/x)
rho_denominator <- 1 + rowMeans (Slack2.yslack/y)
Slack2.rho <- rho_numerator/rho_denominator
```


```{r}
kbl (cbind(Slack1.rho,Slack1.Res, Slack2.rho,Slack2.Res), 
     booktabs=T, escape=F, digits=3,
     col.names=c("$\\rho^{CRS}$", store7names$LambdanamesbyletterLX, 
                 store7names$SXnamesLX, store7names$SYnamesLX,
