...
The value $\phi$ is a radial expansion that unit *k* is expected to make on every output whereas in the input-oriented model, $\theta$ was a radial reduction for every input. A very inefficient unit that received an input-oriented, constant returns to scale efficiency score of $\theta=0.5$ indicates that it should be able to achieve its current output level using no more than half of its inputs. If the same analysis were done using an output-orientation, the unit would get a score of $\phi=2.0$ indicating that using the current input, the unit should at least double each output. In the case of constant returns to scale, the efficiency scores will be consistent ($\theta$=$\frac{1}{\phi}$), but targets will be different. Under other returns to scale assumptions, this relationship between $\theta$ and $\phi$ generally does not hold. The result is that the orientation can have a significant impact on efficiency scores, targets of performance, and who is used to in the developing performance targets for inefficient units.
...
library (TRA)              # My DEA Helper Function
```
...
All we need to do is change the line for `RTS`.
...
The code that we previously developed will then trigger the addition of the VRS constraint in the model building.
...
For the sake of space, I'll omit showing the rest of the code chunk from the previous section but let's compare the results to the CRS case.
...
We have focused on the output-oriented radial efficiency scores up to this point in this chapter. The primary emphasis is on proportionally increasing the outputs while using no more input. We can add a two phase slack maximization to the output-oriented model in the same manner.  

Instead, let's look at using an approach that is often described as non-oriented.  Rather than dealing with slacks at the end and in a second phase, another approach is to use it directly at the beginning. This slack-based model omits radial efficiency measure ($/theta$ or $/phi$) and instead focuses strictly on the slacks. The slacks will describe the total input and output that each inefficient should be producing.  
...
Notice that we have neither $/theta$ nor $/phi$.
...
The following is the input-oriented CRS two-phase approach we developed in Chapter 2.  
...
Now let's switch to the non-oriented Slack based model. Note that it no longer needs to have a variable for the radial efficiency or a need for a second phase.
...
kbl (cbind(CRS2PhaseIO.Res,Slack1.Res), booktabs=T, escape=F, digits=3,
  ...
  add_header_above(c(" "=1, "Efficiency" = 1, "Input-oriented Results" = 10, 
                     "Slack Based Results" = 10))                        |>
...
-   A reminder that the values of the second are on the order of ten times that of the first input. This means that in adding slacks, the second input will tend to have more pull in the objective function.

### Slack Percentages

These observations lead us to consider a couple of options. First, we can convert the sum of slacks to a sum of slack percentages by dividing by each store's actual inputs or outputs. Be careful though to avoid a divide by zero.
...
for (k in 1:ND) {
  ...
  LPSlack <- MIPModel()                                             |>
    ...
    LPSlack<-set_objective(LPSlack, 
                           sum_expr(Vxslack[i]/xdata[k,i], i=1:NX)
                           + sum_expr(Vyslack[r]/ydata[k,r], r=1:NY), 
                           "max")
