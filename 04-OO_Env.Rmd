...
Furthermore, we can come up single metric $\rho$, that can serve as an efficiency score.  While it doesn't have the same radial interpretation of both $\theta$ or $\phi$ from the input or output-oriented model, it provides a nice holistic metric that captures inefficiency on both.  

The following formula for 
$$
\rho = \frac {1-\sum_{i=1}^{N^X} \frac{s^x_i}{x_{i,k}}}  {1+\sum_{r=1}^{N^Y} \frac{s^y_r}{y_{r,k}}}
$$
