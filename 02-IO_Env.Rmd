...
-   The \index{Piping} piping operator `|>` means to pass the previous command's results into the next command. The piping operator requires R 4.1.0 or higher to be used.^[Prior to a piping operator being built in to R, many people used a piping operator, '%>%' from the 'magrittr' package.  They function similarly though for most purpses.]
...
[^02-io_env-2]: In R, as in many languages, the distinction between assignment and testing for equivalency is important. Two symbols are used for assignment, `=` or `<-` while `==` is used to test for whether two items are equivalent. Note that in `ompr` a pair of equals is used for equality constraints. 
