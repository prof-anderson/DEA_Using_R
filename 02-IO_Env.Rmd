...
```{r DEA-IO-Plot1, out.width="50%", fig.align='center', fig.cap="A Simple DEA Model for Store Management"}
...
$$80\cdot\lambda_A + 100\cdot\lambda_B + 300\cdot\lambda_C +400\cdot\lambda_D =  100$$
...
Our goal is to find the *best* target or smallest value of $theta$.  We could then pull this together to say
...
This is a linear program which we could then enter and run.  If we do so, we would find that same of the values of $\lambda$ would be negative.  Using negative values of a store to make a target for performance doesn't really make sense this in application.  A zero value makes sense but not a negative value so we add non-negativity constraints or bounds on the $\lambda values.
