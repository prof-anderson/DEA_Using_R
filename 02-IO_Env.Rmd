...
The piping command command was used previously in making a nicely formatted table but it can also make model building much more concise to not need to keep reassigning things to the model with each command. A few things to keep in mind with piping:
...
For our results, we will then select to use the \index{TRA!LambdanamesbyletterLX} `LambdanamesbyletterLX` to give LaTeX structured column names ready to be used as column names. We can can then apply column names to the table without needing to redefine the original table. Note the use of `escape=F` is needed for it accommodate LaTeX code. We will use a similar approach for many future tables but will often not display the table generating code chunk for brevity by using a code chunk option of `include=FALSE`. The actual source code for each chapter of the book is available from <https://github.com/prof-anderson/DEA_Using_R>.
...
The `ompr` package returns the \index{Dual values!Extraction} of linear programs. Let's now wrap this into the full series of linear programs so as to calculate the \index{Multiplier weights} multiplier weights for each unit in the CRS model. We will discuss the weights in more detail in later chapters.
