...
2.  The text passed to the function such as `"Sales\n($/week)"` allows for a line return character such as `"\n"`. This allows the name of the output and the measurement unit to be on separate lines. The line return is also used to provide a taller box by being placed above and below the label for the box.
...
-   The \index{Piping} piping operator `|>` means to pass the previous command's results into the next command. The piping operator requires R 4.1.0 or higher to be used.[^02-io_env-1]
...
[^02-io_env-1]: Prior to a piping operator being built in to R, many people used a piping operator, '%\>%' from the 'magrittr' package. They function similarly though for most purpses.
...
```{r  warning=FALSE, message=FALSE, fig.cap="Plot of First Data Set"}
...
```{r Fig-Orientation-IO-Projection, echo=FALSE, out.width="60%", fig.show='hold', fig.align='center',fig.cap="Input-Oriented Projection for Unit B", fig.show='hold'}
...
Expressing the target on the left and the actual unit's value with the radial reduction on the right is conceptually straightforward to understand. Standard form is to collect all the terms with variables on the left and place constants (terms without variables) on the right hand side of the inequalities. This is easily done.
...
```{r}
RTS_Table  <- matrix(c("CRS", "$\\text{None or }\\sum_{j=1}^{n} \\lambda_j  \\geq 0$", 
...
kbl (RTS_Table , booktabs=T, escape=F, 
     ...
     caption =c("Returns to Scale Constraints for Envelopment Models"))  |>
  kable_styling (latex_options = c("HOLD_position"))
```

| Returns to Scale | Envelopment Constraint |
|------------------------|------------------------------------------------|
| CRS | No constraint needed or $\sum_{j=1}^{N^D} \lambda_j \geq 0$ |
