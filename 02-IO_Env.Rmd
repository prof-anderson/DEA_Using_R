...
\index{Data envelopment analysis|(} Data Envelopment Analysis (DEA) is a powerful tool for conducting efficiency studies and has been used in thousands of publications since its inception in the 1970s. \index{Efficiency}

While tools exist for conducting these evaluations, it is important to understand their underlying mechanics. Many DEA studies have been conducted and published by authors with only a superficial understanding of the technique. This is equivalent to having a house built by carpenters who barely understand how to use a hammer. The purpose of this document is to explain what DEA is, demonstrate how it works, and how to use R for getting started with DEA. To keep things simple, the first step examines only the \index{Input-oriented} input-oriented \index{Envelopment model} envelopment model with \index{Constant returns to scale} \index{CRS|see{Constant returns to scale}} \index{Constant returns to scale|(} constant returns to scale. We will consider other models soon.
...
The Extreme Technology Analytics research group at Portland State University adopted the R platform based on the following strengths:

- Open-source, so people can dive as deep as they need, with no risk of future vendor lock-in.
...
- One platform that combines optimization, simulation, and statistical analysis.

## An Illustrative Example

DEA typically has \index{Inputs} inputs and \index{Outputs} outputs. Inputs are typically resources consumed in the production of outputs. Inputs are referred to as "bads" because higher input levels for the same output are considered worse. Similarly, holding everything else constant, an increase in any single output is laudable. Examples of inputs might include capital, labor, or the number of machines.
...
```{r DEA-IO-Plot1, warning=FALSE, message=FALSE, eval=TRUE, out.width="60%", fig.align='center', fig.cap="A Simple DEA Model for Store Management"}
library (TRA) # Note, may need to install directly from github
              # remotes::install_github("prof-anderson/TRA")
library (DiagrammeR) # Should not be required because
     # TRA package uses imports DiagrammeR.
...
2.  The text passed to the function such as `"Sales\n($/week)"` allows for a line return character, `"\n"`. This enables the name of the output and the measurement unit to be on separate lines. The leading and edning line returns are also used to provide a taller box by being placed above and below the label for the box.
...
The next code chunk creates a better formatted table. We will use \index{kable} `kable` from the \index{knitr} `knitr` package for displaying the table. We will use the added package \index{kable!kableExtra} `kableExtra` to provide a simpler way to make sophisticated tables. We'll provide the code chunk for this table but generally avoid code chunks for displaying tables in the future unless there is a specific item to show.

The first line of this code chunk ensures the \index{kable!kableExtra} `kableExtra` package is loaded. The \index{kable!kbl} `kbl` command is used to provide a shortcut for the \index{kable} `kable` function with explicit options available. Let's look at several of these options:

- The \index{kable!booktabs} `booktabs` option is for generating clean PDF formatting of the tables required by the publisher.

- The \index{kable!escape} `escape` option allows for LaTeX code to be used as needed when set to `F` or `FALSE`.

- The \index{Piping} piping operator `|>` passes the previous command's results into the next command. [^02-io_env-1]
...
[^02-io_env-1]: Prior to a piping operator being built in to R in version 4.1.0, many people used a piping operator, '%\>%' from the 'magrittr' package. They function similarly though for most purposes.

For more information on using \index{kable} `kable` and \index{kable!kableExtra} `kableExtra` for doing mathematical modeling, see Appendix D of *Optimization Modeling Using R* [@AndersonOMUR2024].

```{r first-dataset-table, warning=FALSE, message=FALSE}
library (kableExtra)
...
```{r  warning=FALSE, eval=FALSE, fig.cap="Plot of First Data Set"}
...
```{r  warning=FALSE, echo=FALSE, fig.cap="Plot of First Data Set"}
library(Benchmarking, quietly=TRUE)
dea.plot(x, y, RTS="crs", ORIENTATION="in-out", 
         txt=LETTERS[1:length(x)], 
         add=FALSE, wx=NULL, wy=NULL, TRANSPOSE=FALSE, 
...
```{r Fig-Orientation-IO-Projection, echo=FALSE, out.width="80%", fig.show='hold', fig.align='center',fig.cap="Constant Returns to Scale Input-Oriented Projection for Unit B", fig.show='hold'}
...
Let's make a simple, explicit linear program for doing DEA. We are going to use the `ompr` package. This package was developed by Dirk Schumacher [@SchumacherOmpr2023] and provides a powerful algebraic optimization modeling tool that we will use this throughout this book.
...
$$ \begin{split}
 \begin{aligned}
    ...
    \text{st  } & 75 \lambda_A + 100 \lambda_B +
...
```{r}
mod2_0 <- add_constraint (mod2_0, 
                          75*VL_A+100*VL_B+300*VL_C+400*VL_D >= 100)
```

Our last `ompr` model building command is to add our input \index{Constraints!Input} constraint.

```{r}
mod2_0 <- add_constraint (mod2_0, 10*VL_A+20*VL_B+30*VL_C+50*VL_D 
                          <= 20*Vtheta)
```
...
There is a lot going on in this code chunk. Let's unpack it step by step.

- The first line takes the solution object, converts it to a matrix, and then transposes it from a \index{Orientation} row orientation to a column orientation. The transpose function is just `t()` and can be thought as switching rows to columns and columns to rows.
...
The use of \index{LaTeX} LaTeX in table headings warrants some explanation. There are several things to bear in mind when using LaTeX rendered in column labels.
...
Let's begin the mathematical development of the envelopment model by asking, can you find a combination of units that produces a target with at least as much output using less input? The blend of other units is described by a vector $\lambda$. Another way to denote this is that $\lambda_j$ is the specific amount of a unit *j* used in setting the target for performance for unit *k*.
...
For simplicity and to allow for a graphical representation, we have focused on the single input and single output case rather than the general *m* input and *s* output case but let's move on to the more general case of multiple inputs and outputs or $m,s\geq1$. To make the code more readable and avoid name space conflicts in R, we will use a slightly different convention $N^X$ or `NX` instead of *m* to refer to the number of inputs (*x*'s) and $N^Y$ or `NY` to be the number of outputs (*y*'s) instead of *s*. Also, the normal mathematical convention is to use *n* to denote the number of \index{Decision making unit} \index{DMU|see{Decision making unit}} Decision Making Units (DMUs) so I will use $N^D$ or `ND` to indicate that in the R code.
...
$$\begin{split}
 \begin{aligned}
    \ & \sum_{j=1}^{N^D} x_{i,j}\lambda_j \leq x_{i,k} \; \forall \; i &\text{[Input]}\\ 
                       & \sum_{j=1}^{N^D} y_{r,j}\lambda_j \geq  y_{r,k} \;  \forall \; r &\text{[Output]}\\
                       & \lambda_j \geq 0  \; \forall \; j
    \end{aligned}
  (\#eq:Ch2ConstructingTargets)
  \end{split}$$

The symbol $\forall$ should be read as "for all". This means that the input \index{Constraints!Input} constraint with $\forall \; i$ is repeated for all values of *i*. Therefore, it would repeat this constraint for values of *i* from *1* to $N^X$. In our single input case ( $N^X=1$ ), we would only have one input constraint. Similarly, $\forall \; r$ in the output constraint indicates that we are repeating this constraint for each possible value of *r* from *1* to $N^Y$. Lastly, we will often use $\forall$ *j* to indicate for all \index{Decision making unit} decision making units from 1 to $N^D$. For the sake of brevity in our linear programming formulations, we will use the $\forall$ *i*, *r*, or *j* when it is for all the possible inputs, or outputs, or decision making units respectively.
...
In this chapter and input-oriented model, we will aim to satisfy the second constraint while trying to improve upon the first constraint by as much as possible. In other words, we will satisfy the second (output) \index{Constraints!Output} constraint but try to form a target that uses as little input as possible.
...
In fact, we will go one step further and say that we want to find the maximum input possible reduction in *k*'s input or conversely, the minimum amount of the input that could be used by the target while still producing the same or more output. We do this by adding a new variable, $\theta$, which is the radial reduction in the amount of DMU *k*'s input. We want to find how low we can drive this by *minimizing* $\theta$. This gives us the following linear program, often abbreviated as LP.[^02-io_env-2]

[^02-io_env-2]: The mathematics in this book are written using LaTeX embedded within the RMarkdown document. Another benefit of using RMarkdown is that it is possible embed LaTeX without requiring using a full LaTeX document.

$$ \begin{split}
 \begin{aligned}
    \text{min    } & \theta \\
    \text{st  } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j \leq \theta x_{i,k} \; \forall \; i &\text{[Input]}\\
                   & \sum_{j=1}^{N^D} y_{r,j}\lambda_j \geq  y_{r,k} \; \forall \; r &\text{[Output]}\\
                   & \lambda_j \geq 0  \; \forall \; j
  \end{aligned}
  \end{split}$$

Expressing the target on the left and the actual unit's value with the radial reduction on the right is conceptually straightforward to understand. On the other hand, the standard form for a linear program is to collect all the terms with variables on the left and place constants (terms without variables) on the right hand side of the inequalities. The input constraint is easily modified to address this by subtracting the right hand side from both sides of the inequality resulting in the following LP.

$$\begin{split}
\begin{aligned}
    \text{min  }   & \theta \\
    \text{st  } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j - \theta x_{i,k} \leq 0 \; \forall \; i &\text{[Input]}\\
                   & \sum_{j=1}^{N^D} y_{r,j}\lambda_j \geq  y_{r,k} \;  \forall \; r &\text{[Output]}\\
                   & \lambda_j \geq 0  \; \forall \; j
  \end{aligned}
 \end{split}$$

### Implementing the Multiple DMU Case
...
Until recently, R did not have the ability to do algebraic modeling optimization but recently a few efforts have provided support for this. The \index{ompr} `ompr` package by Dirk Schumacher [@SchumacherOmpr2023] provides an algebraic perspective that closely matches the summation representation of a linear program shown earlier. If you want to see the data structure format approach, that is covered in the next chapter.

We'll start by exstending to multiple DMUs but still hardcoding for the single input and single output case. Let's define some data structures for holding our data and results.
...
At this point, we now have a complete model. Typing `mod2_1` in the console after running all of the above commands will result in a summary of the model.
...
This tells us that the model was successfully solved to optimality for store B ($k=2$) and that the optimal solution ($\theta$) was 0.50.
...
The piping command was used previously in making a nicely formatted table, but it can also make model building much more concise to not need to keep reassigning things to the model with each command. A few things to keep in mind with piping:
...
- Piped `ompr` models may be more concise looking but do not appear to run significantly faster based on my informal tests. Note that if speed is a concern, `ompr` has a new modeling structure in development, `MILPModel` that would be significantly faster, but this may be more useful for bigger models than DEA typically requires.
...
It starts by generating a set of names based on model dimensions using the \index{TRA!DEAnames} `DEAnames` function from the `TRA` package. The syntax needed for giving names such $\lambda_C$ can be tricky and a lot of typing. The TRA package provides a set of common DEA names given the dimensions of the DEA model. These can then be simply applied as row names or column names as desired. An `LX` suffix is used to differentiate a LaTeX formatted name. More details on this function are provided in the Appendix but to demonstrate the use we'll provide the names that would be used in our 4 store example.

```{r}
res2_1.names <- TRA::DEAnames(NX, NY, ND)
res2_1.names$Lambdanamesbyletter    # Plain text
res2_1.names$LambdanamesbyletterLX  # LaTeX formatted
```

For our results, we will then select to use the \index{TRA!LambdanamesbyletterLX} `LambdanamesbyletterLX` to give LaTeX structured column names ready to be used as column names. We can then apply column names to the table without needing to redefine the original table. Note the use of `escape=F` is needed for it accommodate LaTeX code. We will use a similar approach for many future tables but will often not display the table generating code chunk for brevity by using a code chunk option of `include=FALSE`. The actual source code for each chapter of the book is available from <https://github.com/prof-anderson/DEA_Using_R>.
...
kbl(cbind(res2_1.efficiency, res2_1.lambda), 
    ...
    caption= 'Input-Oriented Envelopment Analysis for Store B') |>
  kable_styling(latex_options = "HOLD_position")
```

The above table follows a few convenient \index{Conventions} conventions. Rather than just using $\theta$, we label the efficiency score by the returns to scale used for the model, in this case constant returns to scale is used and therefore $\theta$ is superscripted with CRS. In some research, it might be labeled as CCR after Charnes, Cooper, and Rhodes [@CharnesDataEnvelopmentAnalysis1978]. Later we will cover other models including the variable returns to scale model and label it as VRS although it is often labeled BCC after Banker, Charnes, and Cooper [@Bankermodelsestimatingtechnical1984].
...
### Extending the Implementation to Multiple Inputs and Outputs

Let's now extend it to handle multiple inputs, `NX`, and outputs, `NY`. This doesn't have any impact on our results just yet since we are still only using a single input and output but we now have the structure to accommodate the more general case. To provide a little variety, we'll change it to the first DMU, A, (`k<-1`) to give a little more variety.

The key change to accommodate multiple inputs is to change the `add_constraint` command to repeat this constraint for `i = 1:NX`. This means that the constraint is repeated for every value from 1 to `NX`. If we had 10 inputs ($N^X=10$) then there would be ten constraints. The first substituting in `i=1`, the second `i=2`, etc. The output constraint is modified in the same manner.
...
Again, the results match what would be expected from Figure 1.

### Solving for all DMUs

Now we will extend our implementation to handle all four of the decision making units by using a `for` loop.
...
The results match those observed graphically but let's discuss them. These results indicate that only DMU *C* is efficient. A rescaled version of *C* could produce the same level of output of *A*, *B*, and *D*, while using 25%, 50%, and 20% less input respectively. The targets of performance for *A* and *C* are constructed by scaling down unit *C* to a much smaller size, as shown by the values of $\lambda$. In contrast, *D*'s performance is surpassed by a 33% larger version of *C*.
...
Instead, let's add a constraint that says that the target of performance must be made up of parts that add up to a whole unit. All it needs to do is constrain the sum of the $\lambda$ variables to equal 1 or in other words, $\sum_{j} \lambda_j = 1$. This prevents the target from being made up of just one greatly shrunken version of a unit, say C making the target for A in our earlier example. It also prevents an over reliance of scaling up a unit to make the performance target, in our example C being scaled up to make a comparison target for D. This constraint makes the model into a variable returns to scale or VRS input-oriented envelopment model.

$$\begin{split}
\begin{aligned}
    \text{min  }   & \theta \\
    \text{st } & \sum_{j=1}^{n} x_{i,j}\lambda_j \leq \theta x_{i,k} \; \forall \; i\\
                       & \sum_{j=1}^{n} y_{r,j}\lambda_j \geq y_{r,k} \; \forall \; r\\
                       & \sum_{j=1}^{n} \lambda_j  = 1 &\text{[VRS constraint]}\\
                       & \lambda_j \geq 0  \; \forall  \; j
  \end{aligned}
  (\#eq:Ch2LPBCCIOE) 
 \end{split}$$

To incorporate this, we can add another constraint to our previous model and solve it. Let's define a parameter, `RTS,` to describe which returns to scale assumption we are using and only add the VRS constraint when `RTS` is set to `"VRS"`.[^02-io_env-3]

[^02-io_env-3]: In R, as in many languages, the distinction between assignment and testing for equivalency is important. Two symbols are used for assignment, `=` or `<-` while `==` is used to test for whether two items are equivalent. Note that in `ompr` a pair of equals is used for equality constraints.
...
Each returns to scale model is straightforward to implement.

```{r, echo=FALSE}
RTS_Table  <- 
  matrix(c("CRS","$\\text{None or }\\sum_{j=1}^{n} \\lambda_j  \\geq 0$",
           "VRS", "$\\sum_{j=1}^{n} \\lambda_j = 1$", 
           "DRS/NIRS", "$\\sum_{j=1}^{n} \\lambda_j \\leq 1$", 
           "IRS/NDRS", "$\\sum_{j=1}^{n} \\lambda_j \\geq 1$"),
           byrow=TRUE, nrow=4)
kbl (RTS_Table , booktabs=T, escape=F, 
     col.names=c("Returns to Scale", "Envelopment Constraint"),
     caption=c("Envelopment Model Returns to Scale Constraints")) |>
...
for (k in 1:ND) {
  ...
  mod2_5 <- MIPModel()                                                |>
    ...
    if (RTS=="VRS") {mod2_5 <-
      add_constraint(mod2_5, sum_expr(Vlambda[j], j = 1:ND) == 1) }  
    if (RTS=="IRS") {mod2_5 <-
      add_constraint(mod2_5, sum_expr(Vlambda[j], j = 1:ND) >= 1) }  
    if (RTS=="DRS") {mod2_5 <-
      add_constraint(mod2_5, sum_expr(Vlambda[j], j = 1:ND) <= 1) }  
...
## A Numerical Example with Multiple Inputs and Outputs

Using DEA for two-dimensional examples such as the one-input, one-output model is easy to draw and visualize but overkill and not generally very useful. Let's extend our earlier store management by adding as an input, store floor space and two more stores.  the floor space might be measured in square meters.    

```{r DEA-IO-Plot2, warning=FALSE, message=FALSE, eval=TRUE, out.width="60%", echo=FALSE, fig.align='center', fig.cap="A Two-Input, One-Output DEA Model for Store Management"}
library (TRA) # Note, may need to install directly from github
              # remotes::install_github("prof-anderson/TRA")
library (DiagrammeR) # Should not be required because
     # TRA package uses imports DiagrammeR.
...
library (rsvg)
tmp<-capture.output(rsvg_png(charToRaw(export_svg(Figure)),
                             'generated-images/DEA_IO_stores2x1.PNG')) 
knitr::include_graphics("generated-images/DEA_IO_stores2x1.PNG")
```
...
kbl(temp, 
    ...
    caption= "Two-Input, Single-Output Grocery Store Model") |>
...
I limited the results to 4 decimal places (`digits=4`) but if I had not, the results might have values such as $\lambda_E=10^{-12}$ in *B*'s evaluation. These numbers should be considered to be zero and are an artifact of the linear programming calculations and numerical methods. The small, nearly zero, values that may appear in DEA calculations can cause some computational difficulty. For example, testing for zero lambda values could miss cases where the value is approximately but not exactly zero.
...
- Stores (DMUs) that were efficient before will stay efficient when shifting to VRS as seen for store *C*.
- Stores (DMUs) that were inefficient, may become efficient such as *A* and *D*. This means that the other stores were not able to demonstrate how to operate better at their size.
- Stores (DMUs) that were inefficient might keep the same efficiency or increase as seen for *B* and *F* but still be inefficient. Their inefficiency is only partly explained by their operating size. 

In summary, switching from CRS to VRS will never hurt the efficiency of a DMU. From an optimization perspective, think of this as adding a constraint which will increase or leave unchanged the objective function value from a minimization linear program. makes it *harder* to find a small value.

## Extracting Multiplier Weights from Envelopment Results

The dual of the envelopment model is called the multiplier model. The dual of the multiplier model is referred to as the multiplier model. These problems are inextricably linked and when one problem is solved (called the primal), the dual is also solved. LP solvers will generally pass the solutions to the dual back with the primal results. We will cover the multiplier model in Chapter 5 but for now, let's use the `ompr` package's feature for returning the \index{Dual values!Extraction} dual values of linear programs. This will give us the solution to the multiplier model while solving the envelopment model.

Let's now wrap this into the full series of linear programs so as to calculate the \index{Multiplier weights} multiplier weights for each unit in the CRS model. We will discuss the weights in more detail in later chapters.
...
kbl (temp,booktabs=T, digits=5, escape=F,
     col.names=c("$\\theta^{CRS}$", store6names$LambdanamesbyletterLX,
        ...
        caption="Results from the Six Store Example")                         |>
...
A few things should be noticed with respect the \index{Multiplier weights} multiplier weights:

- Multiplier weights can be quite small because in general, they are multiplied by inputs and outputs but the product is typically less than one. When inputs or outputs are on the order of millions, then the weights will typically be less than $10^{-6}. They may appear to round to zero but still be significant.
- Multiplier weights are not unique for strongly efficient DMUs. This is explored in more detail in the multiplier model chapter (5) and the cross-efficiency chapter (6).
- Multiplier weights are usually unique for inefficient DMUs and the results obtained using either the \index{Dual values!Row} row duals of the envelopment model or those obtained directly from the multiplier model should therefore  match for those DMUs.
...
resalt <- cbind(get_solution(results, Vtheta), 
              t(as.matrix(as.numeric(
                get_solution(results, Vlambda[j])[,3] ))))
rownames (resalt) <- c("G-Alternate")
```

```{r, echo=FALSE}
Store7CRS.Res <- rbind (Store7CRS.Res, resalt)
# rownames(Store7CRS.Res) <- store7names$DMUnamesbyletter
kbl (Store7CRS.Res, booktabs=T, escape=F, digits=4,
     col.names=c("$\\theta^{CRS}$", store7names$LambdanamesbyletterLX),
        caption="Alternate Optimal Solution for Gary's Store")               |>
   add_header_above(c(" " = 1, "Efficiency" = 1, "Envelopment Variables" = 7)) |>
kable_styling(latex_option="HOLD_position") |>
  row_spec(c(7,8), bold = TRUE)
```

Notice that this is an instance of *multiple optimal solutions* or *multiple optima*. We can demonstrate this by simply adding a constraint that prevents at iteration 7 (Gary's Grocery) from having Trader Carrie's used in target setting ($\lambda_C=0$) will result in exactly the same efficiency score and thereby objective function value. We were lucky in this case to find a target that pointed initially to Trader Carrie's. In the case of multiple optima, finding a solution by luck is not a reliable procedure. We could have just as easily found Gary's Grocery to use itself as the comparator setting target using five extra workers than Trader Carrie's.

### Modeling to Account for Slacks

In order to accommodate this, we need to extend the simple radial model by adding variables to reflect nonradial slacks. We do this by converting the model's \index{Constraints!Input} input and \index{Constraints!Output} output constraints from inequalities into equalities by explicitly defining slack variables. The input slack variable(s), $s^x_i$, represents the amount of input, *i*, can be reduced from the target for the analysis. Similarly, the output slack variable(s), $s^y_r$, indicate the additional amount of output *r* which the target is able to generate. These are considered non-radial slacks and are above and beyond the proportional inefficiency indicated by $\theta$.

$$\begin{split}
 \begin{aligned}
    \text{min }   & \theta \\
    \text{st } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j - \theta x_{i,k} + s^x_i = 0 \; \forall \; i\\
                       & \sum_{j=1}^{N^D} y_{r,j}\lambda_j - s^y_r =  y_{r,k} \; \forall \; r\\
                       & \lambda_j , s^x_i, s^y_r \geq 0  \; \forall \; j,i,r
 \end{aligned}
\end{split}$$

Simply formulating the model with slacks is insufficient. We want to maximize these slacks after having found the best possible radial contraction (minimum value of $\theta$.) This is done by adding a term summing the slacks to the objective function. Note that this sum of slacks is multiplied by $\epsilon$ which is a non-Archimedean infinitesimal. The value of $\epsilon$ is smaller than any positive number but still larger than zero. It should be interpreted to be so small as to ensure that minimizing $\theta$ takes absolute priority or maximizing the sum of slacks. Note also that maximizing the sum of slacks is denoted by minimizing the negative sum of slacks. \index{non-Archimedean infinitesimal}

$$\begin{split}
 \begin{aligned}
    \text{min  }   & \theta - \epsilon ( \sum_{i} s^x_i + \sum_{r} s^y_r)\\
    \text{st  } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j - \theta x_{i,k} + s^x_i = 0 \forall \; i\\
                       & \sum_{j=1}^{N^D} y_{r,j}\lambda_j - s^y_r =  y_{r,k} \forall \; r\\
                       & \lambda_j , s^x_i, s^y_r \geq 0  \; \forall \; j,i,r
  \end{aligned}
...
A common mistake in implementing DEA is to use a finite approximation for $\epsilon$ such as $10^{-6}$. Any finite value can cause distortions in $\theta$. For example, an application comparing companies using revenue and expenses might have inputs and outputs on the order of millions or billions. In this case, non-radial slacks could also be on the order of $10^{6}$ or larger. Multiplying  $10^{-6}$ and the sum of slacks results in a value similar in magnitude to the maximum possible efficiency score ($10^{-6}\cdot 10^{6}=1$) which would then potentially overwhelm the radial efficiency, $\theta$, part of the objective function and lead to distorted results. The impact would be more extreme if had values in the billions!
...
\begin{split}
 \begin{aligned}
    \text{min  }   & \theta &\text{[Regular DEA Model]}\\
    \text{st } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j - \theta x_{i,k} + s^x_i = 0 \; \forall \; i\\
                       & \sum_{j=1}^{N^D} y_{r,j}\lambda_j - s^y_r =  y_{r,k} \; \forall \; r\\
                       & \lambda_j , s^x_i, s^y_r \geq 0  \; \forall \; j,i,r
 \end{aligned}
...
\begin{split}
 \begin{aligned}
    \text{max   } & \sum_{i}s^x_i + \sum_{r}s^y_r &\text{[Maximize Sum of Slacks]}\\
    \text{s.t.: } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j - \theta x_{i,k} + s^x_i = 0 \; \forall \; i\\
                  & \sum_{j=1}^{N^D} y_{r,j}\lambda_j - s^y_r =  y_{r,k} \; \forall \; r\\
                  & \theta = \theta^* &\text{[Keep fixed from Phase 1]}\\
                  & \lambda_j , s^x_i, s^y_r \geq 0  \; \forall \; j,i,r
  \end{aligned}
\end{split}$$

A careful reader might notice that the slacks are being added together despite having very different units. A unit change of each might be of quite different value. We could weight these based on other information but impact would only affect the specific target constructed in the second phase. It wouldn't change the radial efficiency score, $\theta$.  

### Implementing the Slack Maximization Approach

Instead of rebuilding the model for the phase 2, we can just take the previous model and and make a few changes to it.  Implementing this algebraically is quite straightforward.
...
We then constrain variable `Vtheta` to that numerical value.

```{r, eval=FALSE}
add_constraint(LPSlack, Vtheta==phase1obj)   # add constraint to model 
```

Lastly, we change the objective function to maximize the sum of slacks.

```{r, eval=FALSE}
set_objective(LPSlack, 
              sum_expr(Vxslack[i], i=1:NX) +  # sum of input slacks
              sum_expr(Vyslack[r], r=1:NY),   # sum of output slacks
              "max")                         # max instead of min
```
...
for (k in 1:ND) {
  ...
  LPSlack <- MIPModel()                                             |>
    add_variable(Vlambda[j], j = 1:ND, type = "continuous", lb = 0) |>
    add_variable(Vtheta, type = "continuous")                       |>
    add_variable(Vxslack[i], i = 1:NX, type = "continuous", lb = 0) |>
    add_variable(Vyslack[r], r = 1:NY, type = "continuous", lb = 0) |>
    ...
    LPSlack<-set_objective(LPSlack, sum_expr(Vxslack[i], i=1:NX)
                  + sum_expr(Vyslack[r], r=1:NY), "max")
...
These results show store G (Gary's Grocery) is radially efficient, $\theta=1$, and therefore can't make a reduction in all inputs simultaneously while still producing the same level of output. On the other hand, the value $s_1^x=5$ indicates a reduction of the first input can still be done. G is therefore weakly efficent while 

The slacks are more formally described as the non-radial slacks. They may be thought of as the difference between the best target and the actual inputs and outputs of the units studied.
...
1.  Add a fifth unit, E, to the original four store example that produces 400 units output using 30 units of input. Graphically evaluate all five units for their efficiency scores and lambda values. ![](images/coffee-cup.png){width="2.5%"}
2.  Using the graphical approach, interpret the solution in terms of who is doing well, who is doing poorly, and who should be learning from whom. ![](images/coffee-cup.png){width="2.5%"}
3.  Examine the new unit, E, using `R` and `ompr`. ![](images/coffee-cup.png){width="2.5%"}
...
5.  Wrap a for loop around the model to examine every unit. Discuss results. ![](images/coffee-cup.png){width="2.5%"} ![](images/coffee-cup.png){width="2%"}
...
7.  Validate results against a DEA package (ex. `DEAMultiplier`, `TFDEA`, `Benchmarking`, `DJL`, `nonparaeff`) ![](images/coffee-pot.png){width="2.5%"}
8.  Construct an example where Phase 2 increases positive slacks from Phase 1. ![](images/coffee-pot.png){width="2.5%"}
9.  Create "cool" graphs or plots of results. ![](images/coffee-pot.png){width="2.5%"}
