...
# library(bookdown); library(rmarkdown); rmarkdown::render("02-IO_Env.Rmd", "tufte_book2")
library(bookdown); library(c); rmarkdown::render("02-IO_Env.Rmd", "pdf_book")
```
...
![A Simple DEA Model for Store Management](images/DEA_IO_stores.PNG){#fig:DEA_IO_stores}

I find this visual diagram a helpful representation that making a function is helpful. A few things to note about the function as used. First, it creates a graphic image and saves it in a project subdirectory named `images`. If you don't have an images directory, you can create one or modify the statement to remove it. Another nice feature is that the text passed to the function such as `"Sales\\n(\$/week)"` allows for a line return character such as `"\\n"`.  

Now we will create the corresponding data.
...
StoreNames<- c("Al\'s Pantry", 
               ...
               "Trader Carrie\'s", 
               "Dilbertson\'s")
StoreData<-cbind(StoreNames,x,y)
colnames(StoreData)<-c("Store Name", "Employees (x)", 
                  "Sales (y)")

```

Note that the `cbind` function is used to do a column-wise binding of three objects, `StoreNames`, `x` and `y` so that they are side by side.  

```{r}
library(kableExtra)
kbl(StoreData, booktabs=T, caption="First Dataset for DEA")
# kbl is a shorthand for kable
# booktabs is an option for making formal publishing table format
```
...
While we aren't giving a formal introduction to R, let's start by explaining the above commands. The first line `library (kableExtra)` loads a package that extends the more basic `kable` package.  `kableExtra` has a number of improvements including resizing tables, a shorthand notation, and much more. [^io_env-1]

[^io_env-1]: `kableExtra` is one of many thousands of contributed external packages that can be downloaded to extend R. If it does not work to load it using the library, you likely need to install it onto your computer using the `install` command.

The following command creates a matrix that hold the data. The data consists of a concatenated or combined list using the `c` command of four numbers. The number of columns for this matrix is set to one (`ncol=1`) and therefore there are four rows. The matrix has row names based on the first four capital letters and a column name of `x`. By convention, DEA practitioners typically use x to denote input. The \<- symbol is a key function in R and it assigns what is on the right to the object on the left. 

The same is done for creating a set of output, y.

The last command uses the `kbl` function  to display the combined data. 
...
Let's start by doing a simple plot of the data. For now, I'm going to make use of a function in Peter Bogetoft and Lars Otto's `Benchmarking` package which provides a very handy two-dimensional plot in the format often used for showing production. The first line loads the `Benchmarking` package and uses an optional parameter of quietly in order to not display comments that might be generated. The `dea.plot`function takes the data and then a variety of custom options, many of them specific to DEA types of plots. `RTS`represents returns to scale and will be dealt with in more detail later in this chapter as will orientation. The `txt`parameter is used for passing data point labels. In this case, we check the length of the vector `x`, see that it has four letters, and then set `txt` to a list of the first four capital letters, A, B, C, and D. If we had used the `letters` function instead of `LETTERS` and 2:5, it would have given the letters b, c, d, and e. Since the `dea.plot` function is built upon standard R plot routines, the authors wisely allow for parameters to also be be passed. For example, `fex`sets a scaling for the text data labels. For more information on how to use the `Benchmarking` package along with a more economics-oriented treatment of DEA, please refer to the package's help file or [@BogetoftBenchmarkingDEASFA2011].
...
[^io_env-2]: The mathematics in this book is was written using LaTeX embedded within the rmarkdown document. Another benefit of using rmarkdown is that it is possible embed LaTeX without requiring using a full LaTeX document.
...
This tells us that the model was solved to optimality and that the optimal solution ($\theta$ in our case), was 0.50. Building the model up step by step can be helpful but a more concise form of model building connect each command to the previous one using a piping operator, `|>`. One way to think of piping is that if you have two functions, A and B and want to use the result of function A in B, you can do this `A() |> B()` or "Take A's result and pass it into the start of B."
...
-   Piping was first implemented in R by way of packages such as `magrittr` and became so popular that a piping operator, `|>`, was added to the base system of `R` in version 4.1.0 (mid 2021).
-   The pipe command is like glue connecting two commands together. This highlights that you don't want to end with another pipe that expects a different object.
-   Piped models are shorter making them easier to write and express.
-   Piped models may be harder to debug as syntax errors point to the full piped object rather than just one individual line.
...
colnames(res2_1.efficiency) <- c("CCR-IO")
...
colnames(res2_1.lambda) <- c("L_A", "L_B", "L_C", "L_D")

kbl(cbind(res2_1.efficiency, res2_1.lambda), booktabs=T,
       caption= 'Input-Oriented Envelopment Analysis for DMU B')
```

The above table follows a few convenient conventions. First, rather than using $\theta$, we label the efficiency score by the model used for the model, in this case the constant returns to scale is labeled as CCR after Charnes, Cooper, and Rhodes [@CharnesDataEnvelopmentAnalysis1978]. Later we will cover other models including the variable returns to scale model, labeled BCC after Banker, Charnes, and Cooper [@Bankermodelsestimatingtechnical1984]. Another convention is to use "L" in place of the Greek symbol $\lambda$ due to complications with using Greek symbols in R matrix row or column names.
...
k<-1    
# DMU to analyze.  Let's change it to the first unit, DMU A.
...
colnames(res2_2.efficiency) <- c("CCR-IO")
res2_2.lambda <- t(res2_2lambda[3])
 # Takes the third column from the results and transposes 
  #    results to be structured better for later viewing
colnames(res2_2.lambda) <- c("L_A", "L_B", "L_C", "L_D")

kbl (cbind(res2_2.efficiency, res2_2.lambda), booktabs=T,
     caption='Input-Oriented Envelopment Analysis for DMU A')
#library (pander);pander (cbind(res2_2.efficiency, res2_2.lambda), caption="Input-Oriented Envelopment Analysis for DMU A")
...
dimnames(res2_3.efficiency)<-list(DMUnames,"CCR-IO")
dimnames(res2_3.lambda)<-list(DMUnames,Lambdanames)
kbl (cbind(res2_3.efficiency, res2_3.lambda), booktabs=T,
...
Lambdanames <- list("L_A", "L_B", "L_C", "L_D")
DMUnames <- list("A", "B", "C", "D")
dimnames(res2_4.efficiency)<-list(DMUnames,"BCC-IO")
dimnames(res2_4.lambda)<-list(DMUnames,Lambdanames)
kbl (cbind(res2_4.efficiency, res2_4.lambda), booktabs=T,
...
Lambdanames <- list("L_A", "L_B", "L_C", "L_D")
DMUnames <- list("A", "B", "C", "D")
dimnames(res2_5.efficiency)<-list(DMUnames,"BCC-IO")
dimnames(res2_5.lambda)<-list(DMUnames,Lambdanames)
kbl (cbind(res2_5.efficiency, res2_5.lambda), booktabs=T,
        caption="Input-Oriented BCC Model with Option of 
        Alternate Returns to Scale")                                  |>
kable_styling(protect_latex = TRUE)
...
kbl(cbind(XBaker1,YBaker1), booktabs=T,
       caption="Baker Dataset")                                       |>
kable_styling(protect_latex = TRUE)
```
...
kbl (cbind(results.efficiency, results.lambda), booktab=T,
...
In contrast, the second and third column warrant a more careful look. Both of those columns report very small negative numbers. These numbers are due to computational issues in the linear programming solvers and should be interpreted as zero. Rounding the results can make the tables easier for interpretation by the user. 

```{r rounded-table}
kbl (cbind(results.efficiency, results.lambda), 
     booktabs=T, 
        caption=
          "Results from Baker's Example (CRS) with Rounded Values")
```
...
```{r}
kbl (cbind(results.efficiency,BakerCCR.Res), 
     booktabs=T, digits=6,
...
results.vweight[k,] <- -1*rduals[1:NX]
   # Extract first NX row duals assuming 
   #   first set of constraints for inputs
...
kbl(c(results.vweight[k,],results.uweight[k,]),
    booktabs=T,
...
kbl (cbind(results.efficiency, results.lambda, results.vweight, 
              results.uweight),booktabs=T, digits=6, 
...
kbl (cbind(results.efficiency,xdata,ydata,results.lambda), 
     booktabs=T, digits=5,
     caption="Input-oriented Efficiency Results")                   |>
kable_styling(protect_latex = TRUE)
```

The rightmost columns, denoted by "L\_" are the resulting lambda values.
