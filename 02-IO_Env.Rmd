...
The next code chunk creates a better formatted table. We will use `kable` from the `knitr` package for displaying the table. We will use the added package `KableExtra` to provide a simpler way to do sophisticated tables. We'll provide the code chunk for this table but generally avoid code chunks for displaying tables in the future unless there is a specific item to show.

Key items in this code chunk is that we ensure the `kableExtra` package is loaded. The `kbl` command is used to provide a shortcut for the `kable` function with explicit options available. Let's look at several of these options:

-   The `booktabs` option is for generating clean PDF formatting of the tables required by the publisher.

-   The `escape` option allows for LaTeX characters and code to be used as needed when set to `T` or `TRUE`.

-   The piping operator `|>` means to pass the previous command's results into the next command. The piping operator requires R 4.1.0 or higher to be used.
...
For more information on using `kable` and `kableExtra` for doing mathematical modeling, see Appendix D of *Optimization Modeling Using R*.
...
The first line loads the `Benchmarking` package and uses an optional parameter `quietly` in order to not display comments that might be generated. The `dea.plot` function takes the data with a variety of custom options, many of them specific to DEA types of plots:
...
-   Since the `dea.plot` function is built upon standard R plot routines, the authors wisely allow for parameters to also be passed. For example, `fex` sets a scaling for the text data labels.

For more information on how to use the `Benchmarking` package along with a more economics-oriented treatment of DEA, please refer to the package's help file or [@BogetoftBenchmarkingDEASFA2011].
...
The first step is to load a collection of libraries to be used later. The `ompr` package is for optimization and serves as a general human readable format of optimization models that can then interface with a variety of solver engines. The `ROI.plugin.glpk` package is for the specific solver engine, `glpk`, that we used. Other LP solving engines are available and can be used instead.
...
The `MIPModel` command creates an initial, empty model which we name `mod2_0` to represent our initial optimization model in chapter two.
...
Next we can add our output constraint.

```{r}
mod2_0 <- add_constraint (mod2_0, 75*VL_A+100*VL_B+300*VL_C+400*VL_D >= 100)
```

Our last `ompr` model building command is to add our input constraint.
...
-   `res2_0$solution_column_duals`: Dual values of the decision variables (columns).
...
-   `res2_0$additional_solver_output`: This collects detailed information directly from the solver used and is particularly helpful for diagnosing problems when the solver status is "error" or in integer programming optimization problems that are solving slowly.
...
-   The second line sets the number of `digits` to 3, `booktabs` is set again for high quality publication, and `escape` is set to `FALSE` or `F` in order to enable LaTeX for column names.

-   The third and fourth lines of code provide LaTeX symbols for column names.

-   The fifth line sets a caption and then pipes (or passes) the `kable` object to the next command using `|>` piping operator.
...
It starts by generating a set of names based on model dimensions using the `DEAnames` function from the `TRA` package. The syntax needed for giving names such $\lambda_C$ can be tricky and a lot of typing. The TRA package provides a set of common DEA names for a variety of model sizes.

We will then select to use the `LambdanamesbyletterLX` to give LaTeX structured column names ready to be used as column names. We can can then apply column names to the table without needing to redefine the original table. We will often use a similar approach but will not display the table generating code chunk for brevity.
...
The above table follows a few convenient conventions. Rather than just using \$\\theta\$, we label the efficiency score by the returns to scale used for the model, in this case the constant returns to scale is used and therefore $\theta$ is superscripted with CRS. In some research, it might be labeled as CCR after Charnes, Cooper, and Rhodes [@CharnesDataEnvelopmentAnalysis1978]. Later we will cover other models including the variable returns to scale model and label it is as VRS although it is often labeled BCC after Banker, Charnes, and Cooper [@Bankermodelsestimatingtechnical1984].
...
temp <- cbind(res2_1.names$DMUnamesbyletter, # Add letters as column
              res2_4.efficiency, 
              format (res2_4.lambda, digits=3, drop0trailing = TRUE)) 
kbl(temp, 
...
-   Multiplier weights are usually unique for inefficient DMUs and the results obtained using either the row duals of the envelopment model or those obtained directly from the multiplier model should therefore typically match.
...
In order to accommodate this, we need to extend the simple radial model by adding variables to reflect nonradial slacks. We do this by converting the model's input and output constraints from inequalities into equalities by explicitly defining slack variables. The slack variable(s) $s^x_i$ represents the amount of input, *i*, can be reduced from the target for the analysis. Similarly, the slack variable(s) $s^y_r$ indicate the additional amount of output r which the target is able to generate. These are considered non-radial slacks and are above and beyond the proportional inefficiency indicated by $\theta$
...
1.  Add a fifth unit, E, that produces 400 units output using 30 units of input. Graphically evaluate all five units for their efficiency scores and lambda values. *Difficulty: Decaf cup of coffee*
2.  Using the graphical approach, interpret the solution in terms of who is doing well, who is doing poorly, and who should be learning from whom. *Difficulty: Cup of coffee*
3.  Examine the new unit, E, using `R` and `ompr`. *Difficulty: Decaf cup of coffee*
...
7.  Validate results against a DEA package (ex. `DEAMultiplier`, `TFDEA`, `Benchmarking`, `DJL`, `nonparaeff`) *Difficulty: Pot of coffee*
