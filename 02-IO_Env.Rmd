...
While tools exist for conducting the evaluations, it is important to understand how the tools work. Many \index{Data envelopment analysis!studies} DEA studies have been conducted and published by authors with only a superficial understanding of the technique. This is equivalent to having a house built by carpenters who barely understand how to use a hammer. The purpose of this document is to explain what DEA is, demonstrate how it works, and how to use R for getting started with DEA. In order to keep things simple, the first step only looks at the \index{Data envelopment analysis!input-oriented} input-oriented envelopment model with \index{CRS!Constant returns to scale} constant returns to scale. We will consider other models soon.

This chapter walks through how DEA works and then shows how to implement the model in R using two very different approaches. Over the years, I have built \index{Data envelopment analysis!models} DEA models in many languages and platforms: Pascal, LINDO, LINGO, Excel Macros, Excel VBA, GAMS, AMPL, XPress-MOSEL, and GLPK among others.
...
The figure for drawing \index{Data envelopment analysis!input-output diagram} input-output diagrams is worth a little discussion and explanation.
...
-   The \index{kable!LaTeX escape} `escape` option allows for LaTeX characters and code to be used as needed when set to `T` or `TRUE`.
...
-   The `kable_styling` command provides a lot more formatting richness. In this case, it is passing an option of `hold_position` to ensure that LaTeX keeps the table placed close to this location.
...
Let's start by doing a simple plot of the data. For now, I'm going to make use of a function in Peter Bogetoft and Lars Otto's \index{Benchmarking!package} `Benchmarking` package which provides a very handy two-dimensional plot in the format often used for showing production.

The first line loads the \index{Benchmarking!package} `Benchmarking` package and uses an optional parameter `quietly` in order to not display comments that might be generated. The \index{dea.plot} `dea.plot` function takes the data with a variety of custom options, many of them specific to \index{Data envelopment analysis!plots} DEA types of plots:

-   `RTS` represents returns to scale and will be dealt with in more detail later in this chapter as will orientation.
...
For more information on how to use the \index{Benchmarking!package} `Benchmarking` package along with a more economics-oriented treatment of DEA, please refer to the package's help file or [@BogetoftBenchmarkingDEASFA2011].
...
This chart shows that unit *C* has the best ratio of output (y) to input (x). The diagonal line represents an efficiency frontier of best practices that could be achieved by scaling up or down unit *C*. As the input is scaled up or down, it is assumed that the output of unit *C* would be scaled up or down by the same proportion. We will revisit this assumption in a later section but for now, think of this as saying that unit *C* cannot enjoy economies of scale by getting larger or suffer from diseconomies of scale by getting smaller so it is referred to as constant returns to scale or CRS.
...
Next, since we are trying to find the smallest possible value of $\theta$, we could in fact relax the equality \index{Constraints} constraints to be inequalities. For the output, the target for B must be at least as large as the actual output of B. For the input constraint we could say that the target input for B cannot exceed the reduced input used by B. For brevity, we can also abbreviate *minimize* as *min* and *subject to* as *s.t.*. This gives us the following linear program formulation.
...
Let's make a simple, explicit \index{Data envelopment analysis!linear program} linear program for doing DEA. We are going to use the `ompr` package.
...
The first step is to load a collection of libraries to be used later. The \index{ompr} `ompr` package is for optimization and serves as a general human readable format of optimization models that can then interface with a variety of solver engines. The \index{ROI.plugin.glpk} `ROI.plugin.glpk` package is for the specific solver engine, \index{glpk} `glpk`, that we used. Other LP solving engines are available and can be used instead.
...
We can see that we have five variables. Next, let's add our objective function (*min* $\theta$).

```{r}
mod2_0 <- set_objective(mod2_0, Vtheta, "min")
```

Next we can add our output \index{Constraints!output} constraint.
...
-   `res2_0$solution_column_duals`: Dual values of the \index{Decision variables} decision variables (columns).

-   `res2_0$solution_row_duals`: Dual values of the constraints (rows). In the DEA envelopment model, these can be used to find the solution to the DEA multiplier model.
...
-   The first line takes the solution object, converts it to a matrix, and then transposes it from a row orientation to a column orientation.

-   The second line sets the number of \index{kable!digits} `digits` to 3, \index{kable!booktabs} `booktabs` is set again for high quality publication, and \index{kable!LaTeX escape} `escape` is set to `FALSE` or `F` in order to enable LaTeX for column names.
...
This gives us two sets of \index{Constraints} constraints along with a restriction of non-negativity. These are shown in the following relationships that must all be satisfied simultaneously.
...
It starts by generating a set of names based on model dimensions using the \index{TRA!DEAnames} `DEAnames` function from the \index{TRA} `TRA` package. The syntax needed for giving names such $\lambda_C$ can be tricky and a lot of typing. The TRA package provides a set of common DEA names for a variety of model sizes.
...
Again, the results match what would be expected from Figure 1. Now we will extend this to handle all four of the \index{Decision making unit} decision making units by using a `for` loop.
...
It is an important step of any \index{Data envelopment analysis!results} DEA study to carefully examine the results. In this case, it might be argued that for certain applications, *C*'s business practices do not scale linearly and therefore could not be assumed to operate at a much smaller or larger size. In that case, we should consider modeling returns to scale.
...
To incorporate this, we can add another \index{Constraints} constraint to our previous model and solve it. Let's define a parameter, `RTS,` to describe which returns to scale assumption we are using and only add the VRS constraint when `RTS` is set to `"VRS"`.[^02-io_env-2]
...
No envelopment \index{Constraints} constraint on the sum of $\lambda$ is needed to create a CRS model and to convert a VRS formulation into a CRS we can just delete the new constraint. On the other hand, it may sometimes be helpful to maintain a consistent model size in terms of number of constraints as we make richer models. We can make it a redundant \index{Constraints!redundant} constraint by constraining the sum of $\lambda$ to be greater than or equal to zero, $\sum_{j} \lambda_j\geq0$. Since $\lambda$'s are by definition non-negative, the sum of $\lambda$'s is also non-negative and therefore the constraint is superfluous or redundant.
...
The small, nearly zero, values that may appear in \index{Data envelopment analysis!calculations} DEA calculations can cause some computational difficulty. For example, testing for zero lambda values could miss cases where the value is approximately but not exactly zero.
...
-   Switching from CRS to VRS will never hurt the efficiency of a DMU. From an optimization perspective, think of this as adding a \index{Constraints} constraint which will increase or leave unchanged the objective function value from a minimization linear program. makes it *easier* for a find

## Extracting Multiplier Weights from Envelopment Results

Recently, Dirk Schumacher added the option of extraction dual results from glpk-solved linear programs. Let's now wrap this into the full series of linear programs so as to calculate the multiplier weights for each unit in the CRS model.
...
-   Multiplier weights are usually unique for inefficient DMUs and the results obtained using either the \index{Row duals} row duals of the envelopment model or those obtained directly from the multiplier model should therefore typically match.
...
Notice that this is an instance *multiple optimal solutions* or *multiple optima*. We can demonstrate this by simply adding a \index{Constraints} constraint that prevents at iteration 7 (Gary's Grocery) from having Trader Carrie's used in target setting ($\lambda_C=0$) will result in exactly the same efficiency score and thereby objective function value. We were lucky in this case to find a target that pointed initially to Trader Carrie's. In the case of multiple optima, finding a solution by luck is not a reliable procedure. We could have just as readily easily found Gary's Grocery to use itself as the comparator setting target using five extra workers than Trader Carrie's.
