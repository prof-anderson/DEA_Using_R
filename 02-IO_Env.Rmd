...
-   Extensive collection of numerical add-in packages (over 10,000 at CRAN) to build upon
...
x <- matrix (c(10,20,30,50), 
             ncol=1, dimnames=list(LETTERS[1:4],"x"))

y <- matrix (c(75,100,300,400), 
             ncol=1, dimnames=list(LETTERS[1:4],"y"))

pander(cbind(x,y), caption="First Dataset for DEA")
```

While we aren't giving a formal introduction to R, let's start by explaining the above commands. The first line `library (pander)` loads a common library that makes for nicely formatted tables. [^io_env-1]

[^io_env-1]: Pander is one of many thousands of contributed external packages that can be downloaded to extend R. If it does not work to load it using the library, you likely need to install it onto your computer using the `install` command.

The following command creates a matrix that hold the data. The data consists of a concatenated or combined list using the `c` command of four numbers. The number of columns for this matrix is set to one (ncol=1) and therefore there are four rows. The matrix has row names based on the first four capital letters and a column name of `x`. The \<- symbol is a key function in R and means to assign what is in the right to the object on the left. By convention, DEA practitioners typically use x to denote input.

The same is done for creating a set of output, y.

The last command uses the pander function display to display the combined data. Note that the `cbind` function is used to do a column-wise binding of the two objects, `x` and `y` so that they are side by side.
...
pander(cbind(res2_1.efficiency, res2_1.lambda), 
       caption=
         "Input-Oriented Envelopment Analysis for 
       DMU B (CCR-IO)")
```
...
pander(cbind(res2_2.efficiency, res2_2.lambda), 
       caption="Input-Oriented Envelopment Analysis for DMU A")

```

Again, the results match what would be expected from Figure 1. Now we should extend this to handle all four of the decision making units.
...
pander (cbind(res2_3.efficiency, res2_3.lambda),
...
To incorporate this, we can add another constraint to our previous model and solve it. Let's define a parameter, "RTS" to describe which returns to scale assumption we are using and only add the VRS constraint when `RTS="VRS"`.[^io_env-3]
...
pander (cbind(res2_4.efficiency, res2_4.lambda), 
...
pander (cbind(res2_5.efficiency, res2_5.lambda), 
