...
\begin{aligned}
    ...
    \text{subject to } & \theta_{j,k}=\frac{\sum_{r=1}^{N^Y} u_{r,k} y_{r,j}} {\sum_{i=1}^{N^X} v_{i,k} x_{i,j} }
                          \leq 1 \; \forall \; j\\
...
Another approach is called the slack-based model.
...
\begin{split}
 \begin{aligned}
    ...
    \text{subject to } & \sum_{j=1}^{N^D} x_{i,j}\lambda_j + s^x_i = x_{i,k}  \; \forall \; i\\
...
\begin{split}
 \begin{aligned}
    ...
    \text{subject to } & 1+\frac{1}{N^Y}\sum_{r=1}^{N^Y} \frac{s^y_r}{y_{r,k}} =1 \\
