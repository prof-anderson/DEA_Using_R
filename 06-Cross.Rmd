...
As discussed in the chapter on the Multiplier model, DEA often has multiple optima, particularly for efficient DMUs. While this doesn't affect the efficiency scores in either the envelopment or the multiplier model, it can have a major impact on the cross-evaluation scores and therefore the cross-efficiency scores.
...
kbl (rbind(t(cbind(x,y)), 
     ...
     booktabs = T, digits=4, drop0trailing = TRUE,
...
```{r}
res <- rbind (SpringLres$ceva_matrix, SpringLres$ceva_max, SpringLres$ce_ave)
resrows <-cbind (c("A", "B", "C", "D", "E", "F", "G", "H", 
         "Efficiency", "Cross-Efficiency"))
kbl (cbind(resrows, as.data.frame(res,digits=5)), 
     row.names=F, booktabs = T, digits=5, escape=F,
