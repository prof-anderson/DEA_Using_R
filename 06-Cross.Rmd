...
library(pander)           # For better tables
...
pander (c(resfunc$efficiency,resfunc$vweight,resfunc$uweight), 
        caption="Cross-Efficiency Scores and weights")
```
...
pander(results.crosseff, caption = "results.crosseff?")
...
pander(rescross$crosseval, "Cross-Evaluation Matrix")
pander(rescross$crosseff, "Cross-Efficiency Scores")
...
#pander(cbind(xdata,ydata),caption="Data Passed to function")
#pander(xdata, caption="xdata Passed to function")
...
XTT <- matrix(c(1,1,1,1,1,1,1),ncol=1,dimnames=list(LETTERS[1:7],"x"))

YTT <- matrix(c(5,7,8,4,9,10,4,10,10,7,8,4,2,7,3,3,5,3,4,10,7),ncol=3,dimnames=list(LETTERS[1:7],c("C1","C2","C3")))
```

Comments about cross-efficiency model.

* Mal_and_Ben is not defined as a function.
* CrossEfficiniency funciton should allow secondary objective function choice of "Malevelent" or "Benevelent"
* Results from CrossEfficiency do not Takamura and Tone. CE Matrix 
...
* Need to add row and column names along with captions for tables to make sense.
