...
Next let's implement this in ompr.  This will serve as a review for ompr.  
...
pander(c(alfred_score, u1_result, u2_result), caption="Alfred's Score and Results")
```
...
pander(c(barb_score, u1_result, u2_result), caption="Barb's Score and Weights")
```
...
u1_result <- get_solution(result, u1) 
u2_result <- get_solution(result, u2) 
barb_score <- 42 * u1_result + 1050 * u2_result

pander(c(barb_score, u1_result, u2_result), caption="Chris's Score and Weights")
```
...
\begin{aligned}
    ...
    \text{subject to } &  \sum_{r=1}^{2} u_r y_{r,1} \leq 1.0 [Alfred]\\
     &  \sum_{r=1}^{2} u_r y_{r,2} \leq 1.0 [Barb]\\
     &  \sum_{r=1}^{2} u_r y_{r,3} \leq 1.0 [Chris]\\
     &  \sum_{r=1}^{2} u_r y_{r,4} \leq 1.0 [Don]\\
     &  \sum_{r=1}^{2} u_r y_{r,5} \leq 1.0 [Ed]\\
     &  \sum_{r=1}^{2} u_r y_{r,6} \leq 1.0 [Fran]\\
     &                  u_1, u_2\geq 0  
  \end{aligned}
\end{split}
...
Now we will extend this further to reflect that the constraint of being less than or equal to 1.0 is the same for every manager.  A key notation to learn is the $\forall$ which means to repeat for all possible values of the index.  
...
Now, let's further extend this model.  Not only are the restaurant managers producing different outcomes, they are also using different resources, say capital and labor.  Let's denote capital investment as $x_1$ and labor in full-time-equivalents as $x_2$.  
...
For the two-input, two-output case, the formulation is given below.  h
$$
\begin{split}
\begin{aligned}
...
We're going to use our data from earlier.  For this example, we will use the dataset from Kenneth Baker's third edition of _Optimization Modeling with Spreadsheets_, pages 175-178, Example 5.3 titled "Hope Valley Health Care Association." In this case, a health care organization wants to benchmark six nursing homes against each other.y
