...
\begin{split}
\begin{aligned}
    \text {max }   & 25 \cdot u_1 + 3500 \cdot u_2 &\text{[Max Alfred's Score]}\\
    \text{s.t.:  } & 25 \cdot u_1 + 3500 \cdot u_2 \leq 1.0&\text{[Alfred' Score]}\\
                   & 14 \cdot u_1 + 2100 \cdot u_2 \leq 1.0&\text{[Barb's Score]}\\
                   & 42 \cdot u_1 + 1050 \cdot u_2 \leq 1.0&\text{[Chris's Score]}\\
                   & 18 \cdot u_1 + 4200 \cdot u_2 \leq 1.0&\text{[Don's Score]}\\
                   & 19 \cdot u_1 + 2500 \cdot u_2 \leq 1.0&\text{[Ed's Score]}\\
                   & 14 \cdot u_1 + 1500 \cdot u_2 \leq 1.0&\text{[Fran's Score]}\\
                   & u_1,\; u_2\geq 0  \
  \end{aligned}
\end{split}
$$
...
result <- MIPModel ()                         |>
  add_variable (Vu1, type="continuous", lb=0)  |>
  add_variable (Vu2, type="continuous", lb=0)  |>
  set_objective (25*Vu1+3500*Vu2, "max")        |>
  add_constraint (25*Vu1+3500*Vu2 <= 1.0)       |>
  add_constraint (14*Vu1+2100*Vu2 <= 1.0)       |>
  add_constraint (42*Vu1+1050*Vu2 <= 1.0)       |>
  add_constraint (28*Vu1+4200*Vu2 <= 1.0)       |>
  add_constraint (19*Vu1+2500*Vu2 <= 1.0)       |>
  add_constraint (14*Vu1+1500*Vu2 <= 1.0)       |>
  solve_model(with_ROI(solver = "glpk"))

u1_result <- get_solution(result, Vu1) 
u2_result <- get_solution(result, Vu2) 
...
result <- MIPModel ()                         |>
  add_variable (Vu1, type="continuous", lb=0)  |>
  add_variable (Vu2, type="continuous", lb=0)  |>
  set_objective (14*Vu1+2100*Vu2, "max")        |>
  add_constraint (25*Vu1+3500*Vu2 <= 1.0)       |>
  add_constraint (14*Vu1+2100*Vu2 <= 1.0)       |>
  add_constraint (42*Vu1+1050*Vu2 <= 1.0)       |>
  add_constraint (28*Vu1+4200*Vu2 <= 1.0)       |>
...
result <- MIPModel ()                           |>
  add_variable (Vu1, type="continuous", lb=0)   |>
  add_variable (Vu2, type="continuous", lb=0)   |>
  set_objective (42*Vu1+1050*Vu2, "max")        |>
  add_constraint (25*Vu1+3500*Vu2 <= 1.0)       |>
  add_constraint (14*Vu1+2100*Vu2 <= 1.0)       |>
  add_constraint (42*Vu1+1050*Vu2 <= 1.0)       |>
  add_constraint (28*Vu1+4200*Vu2 <= 1.0)       |>
...
less than or equal to 1.0 is the same for every manager. 
...
Note that this model does not incorporate the resources
...
therefore equally valid. One clever trick solves the remaining non-linear objective function by
setting the denominator to be a constant. Essentially we are selecting a
...
$\sum_{i=1}^{N^X} v_i x_{i,k}=1$, to the linear program and replace the
...
for (k in 1:ND) {
  ...
  set_objective(sum_expr(Vuweight[r] * ydata[k,r], r = 1:NY), "max") |>
  add_constraint(sum_expr(Vvweight[i] * xdata[k,i], i = 1:NX) == 1)  |>
  add_constraint((sum_expr(Vuweight[r] * ydata[j,r], r = 1:NY)-
                    sum_expr(Vvweight[i] * xdata[j,i], i = 1:NX)) 
                 <= 0, j = 1:ND)
  result
...
```{r}
rownames(mm1.combined) <- c("A", "B", "C", "D", "E", "F")
...
Note that the efficiency scores and the $\lambda$ values match those of the six store example from chapter 2.  
...
For example, we could simply indicate that each unit of the second output is more valuable or challenging to produce than the first. We could then add a constraint that $u_1\leq u_2$.  If we had more specific information, we might put a range on that relationship such as the weight of the second output must be at least 10% of the first output and no more than double that of the first output.  This would be represented as $0.1\leq \frac{u_2}{u_1} \leq 2.0$ which is readily linearized by muliplying out the $u_1$ term.
...
for (k in 1:ND) {
  ...
  add_constraint ((sum_expr(Vuweight[r] * ydata[j,r], r = 1:NY)-
                    sum_expr(Vvweight[i] * xdata[j,i], i = 1:NX)) 
                 <= 0, j = 1:ND)                                     |>         
  add_constraint (Vvweight[1]>=2*Vvweight[2])  # Weight Restriction
  ...
  # Get the output weights
  tempvweight <- get_solution(mm2result, Vvweight[i])
  mm2.vweight[k,] <- tempvweight[,3]

  # Get the input weights
  tempuweight <- get_solution(mm2result, Vuweight[i])
...
mm2.combined <- cbind (mm1.efficiency, mm1.vweight, mm1.uweight,
                       mm2.efficiency, mm2.vweight, mm2.uweight)
```

```{r, include=FALSE}
rownames(mm2.combined) <- c("A", "B", "C", "D", "E", "F")
...
The linearization follows in a similar process to the input-oriented model. This is the dual of the output-oriented CRS envelopment model in chapter 4. We can also reinforce this by denoting the objective function with a $\phi$ since it will match the radial efficiency score from the envelopment model.  

$$
\begin{split}
\begin{aligned}
    \text {min }  &  \phi=\sum_{i=1}^{N^X} v_i x_{i,k}\\
...
this variable $\mu$ to reflect the returns to scale associated with
this DMU. If we are implementing a VRS model, then the equality
constraint indicates that $\mu$ is an unrestricted (or free) variable. To highlight that the scale variable is relative to the DMU being analyzed at that stage, we will subscript it with a *k* as $\mu_k$. 
...
The output-oriented VRS multiplier model is the following.  The returns to scale term is now denoted as *nu* or $\nu_k$ for unit *k*.  Note that it is common in some DEA multiplier models to use the Greek symbols nu ($\nu$) and mu ($\mu$) as the input and output multipliers (weights) instead of *v* and *u* and then use the alternate as the returns to scale terms.  In this book I use *u* and *v* consistently for the  multipliers to make it more accessible for the reader and limit confusion between what appear to be visually similar terms *v* and $\nu$ to this this section. Again, the  $\nu$ is subscripted with a *k*.
...
### Multiplier Model Numerical Example 
...
for (k in 1:ND) {
  ...
  add_variable (Vvweight[i], i = 1:NX, type = "continuous", lb = 0)   |>
  add_variable (Vuweight[r], r = 1:NY, type = "continuous", lb = 0)   |>
  add_variable (Vmu, type = "continuous")                             |>
  set_objective (sum_expr(Vuweight[r] * ydata[k,r], r = 1:NY)+Vmu, 
                 "max")                                               |>
  add_constraint (sum_expr(Vvweight[i] * xdata[k,i], i = 1:NX) == 1)  |>
  add_constraint ((sum_expr(Vuweight[r] * ydata[j,r], r = 1:NY)-
                    sum_expr(Vvweight[i] * xdata[j,i], i = 1:NX))+Vmu 
                 <= 0, j = 1:ND)                                             
  ...
  # Get the output weights
  tempvweight <- get_solution(mm3result, Vvweight[i])
  mm3.vweight[k,] <- tempvweight[,3]
  mm3.mu[k] <- get_solution(mm3result,Vmu)
  # Get the input weights
  tempuweight <- get_solution(mm3result, Vuweight[i])
...
mm3.combined <- cbind (mm1.efficiency, mm1.vweight, mm1.uweight,
                       mm3.efficiency, mm3.mu, mm3.vweight, mm3.uweight)
```

```{r echo=FALSE}
rownames(mm3.combined) <- c("A", "B", "C", "D", "E", "F")
kbl (mm3.combined , booktabs=T, escape=F, digits=5,
     col.names=c("$\\theta^{CRS}$", mm1names$VnamesLX,
                 mm1names$UnamesLX,
                 "$\\theta^{VRS}$", "$\\mu_k$", mm1names$VnamesLX,
                 mm1names$UnamesLX),
     caption =c("Multiplier Model Results from Grocery Store with CRS vs. VRS")) |>
  add_header_above(c(" "=1, "Original Results" = 4, 
...
kbl (RTS_Table , booktabs=T, escape=F, 
     col.names=c("Returns to Scale", "Envelopment", "Input-Oriented\nMultiplier", "Output-Oriented\nMutiplier"),
...
If $\theta_k^*=1$ and $Q_k>0$ then unit *k* is weakly effficient.  \index{Slack|)}

## Super-Efficiency in the Multiplier Model

Super-efficiency is a DEA variant that can be readily understood from the ratio model. The goal is still to maximize a store's score but now only the other scores are limited to getting 1.0 or less. This means that efficient stores may get scores over 1.0 or 100%!  

In the ratio model, we don't require that unit *k* limits its own score to 1.0.  This is implemented by simply eliminating the ratio constraint for that DMU or store, i.e. where $j=k$. \index{Super-efficiency}
...
Super-efficiency has been justified for a variety of reasons including ranking the efficient units, breaking the *ties* between the efficient units, and to potentially provide better statistical properties for post efficiency analysis by eliminating the spike at 1.0.

Care must be taken with super-efficiency though for multiple reasons. Super-efficiency LPs can be infeasible, particularly in returns to scale models beyond CRS. Also, super-efficiency values are a measure of how different the efficient unit is from others which is not the same as saying that they are the best of the best.  

One of the underappreciated uses of super-efficiency is as a diagnostic for a DEA model and the data. A super-efficiency of 1.1 ($/theta^{SE}_{IO-CRS}=1.1) may be impressive as it indicates that it is outperforming its similar peers by 10%.  On other hand, a super-efficiency of 1.5 means that it is outperforming peers by 50% which is a tremendous difference. This impressive level of performance should be readily understood by knowledgeable industry experts. On the other hand, if the experts can point out why that DMU is not actually so impressive, perhaps the model needs to be revised. In fact, published journal papers have sometimes reported a super-efficiency of 6.0 or higher. This means that the unit is 500% or better than others based on the weights it uses. Perhaps this means that the dataset has an error, the input-output model needs to be reconsidered, or perhaps weight restrictions can eliminate extreme over valuing of an output.

```{r Structure-MM4-Super-Efficiency}

mm4.efficiency <- matrix(rep(-1.0, ND), nrow=ND)
mm4.lambda     <- matrix(rep(-1.0, ND^2), nrow=ND)
mm4.vweight    <- matrix(rep(-1.0, ND*NX), nrow=ND) 
mm4.uweight    <- matrix(rep(-1.0, ND*NY), nrow=ND) 
mm4.xslack     <- matrix(rep(-1.0, ND*NX), nrow=ND) 
mm4.yslack     <- matrix(rep(-1.0, ND*NY), nrow=ND) 

mm4names <- TRA::DEAnames(NX, NY, ND)
```
...
for (k in 1:ND) {
  ...
  mm4result <- solve_model(mm4result, with_ROI(solver = "glpk", 
                                         verbose = FALSE))
  mm4.efficiency[k] <- objective_value (mm4result) 

  # Get the output weights
  tempvweight <- get_solution(mm4result, Vvweight[i])
  mm4.vweight[k,] <- tempvweight[,3]

  # Get the input weights
  tempuweight <- get_solution(mm4result, Vuweight[i])
  mm4.uweight[k,] <- tempuweight[,3]
  templambda <- as.matrix(get_row_duals(mm4result))
  mm4.lambda[k,] <- templambda [2:7]
  # Drops the first and last row duals as those constraints
  #   since the first is for setting demon=1, last is for 
  #   weight restrictions.
 }

mm4.combined <- cbind (mm1.efficiency, mm1.vweight, mm1.uweight,
                       mm4.efficiency, mm4.vweight, mm4.uweight)
```


```{r echo=FALSE}
rownames(mm4.combined) <- c("A", "B", "C", "D", "E", "F")
kbl (mm4.combined , booktabs=T, escape=F, digits=5,
     col.names=c("DMU", "$\\theta^{CRS}$", mm1names$VnamesLX,
                 mm1names$UnamesLX,
                 "$\\theta^{CRS-SE}$", mm1names$VnamesLX,
                 mm1names$UnamesLX),
     caption =c("Super-Efficiency Results from Grocery Store Example")) |>
  add_header_above(c(" " = 1, "Original Results" = 4, 
                     "Super-Efficiency" = 4))                   |>  
  kable_styling (latex_options = c("HOLD_position","scale_down")) |>
...
In this case, the super-efficiency, *C*'s super-efficiency is 1.36 meaning that it is able to produce the same or more output while using 36% less of all of the inputs than the best target for performance for *C*. If we had a weakly efficient DMU, it would have still 1.0.

Super-efficiency can be implemented in the envelopment model by eliminating the option of comparing against oneself, by either eliminating the $\lambda_k$ variable or more simply setting  it to zero, ($\lambda_k=0$).
