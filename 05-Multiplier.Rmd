...
Remember the inputs are named as "x1" and"x2" to represent the staff (full-time equivalent employees) and the store size ($m^2$) respectively. The output is sales are named "y1". As usual, I'm copying the source data into objects, `xdata` and `ydata`, for actual operation.
...
We are now ready to do the analysis. In chapter 2 we used the envelopment model to examine this case. Now we will use the multiplier model.
...
#      caption="Multiplier Model Results from 7 Store Example")
...
kbl (mm1.combined , booktabs=T, escape=F, digits=4,
     col.names=c("$\\theta^{CRS}$", mm1names$VnamesLX, mm1names$UnamesLX, mm1names$LambdanamesbyletterLX),
...
### Envelopment Multipler Model Duality Relationship

Duality is a fundamental characteristic of linear programming.  Every linear program has a dual program that is a bit of mirror image of the original.  The multiplier model and the envelopment models are fundamentally connected by duality. The multiplier model of DEA is considered the dual of the envelopment model and the reverse is also true.
...
- Using the *C* vector as the right hand side values of the constraints.
...
We won't dwell further on this transformation and will leave it to the reader to explore further but this is is why we can extract dual values of the envelopment model to get the multiplier weights from the multiplier model.  Also, it is why we can extract the envelopment's $\lambda$ values from the multiplier model's results.   
...
Formulate and implement a model
Implement model
