...
The linear program for the input-oriented envelopment model of a DMU _k_ is to find a target constructed of a mix of the DMU's described by $\lambda_{j}$ that use no more input to achieve the same or more output.  The amount of the input used is $\sum_{j=1}^{n} x_{i,j}\lambda_j$.  


This needs to be less than i'th input of DMU $x_{i,k}$. 
...
#  dimnames(results.efficiency)<-list(c(LETTERS[1:ND]),c("Eff"))
  dimnames(results.efficiency)<-c(DMUnames,"Eff")  # Attach names
...
omptheta <-  get_solution(result, vtheta) 
omprlambda <-  get_solution(result, vlambda[j])
...
pander (results.efficiency, caption="Input-Oriented Efficiency Scores")
pander (results.lambda, caption="Lambda Values")
...
pander (results.efficiency, caption="Efficiency Scores")
pander (results.lambda, caption="Lambda Values")
...
dimnames(results.efficiency)<-c(DMUnames,"Eff")
dimnames(results.lambda)<-c(DMUnames,DMUnames)
dimnames(results.vweight)<-c(DMUnames,Vnames)
dimnames(results.uweight)<-c(DMUnames,Unames)
dimnames(results.xslack)<-c(DMUnames,SXnames)
...
pander(cbind(results.efficiency,xdata,ydata), round=5, caption="Input-oriented Efficiency Results")
