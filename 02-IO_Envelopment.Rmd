...
Recently, ompr has been released to CRAN which makes installation much easier.  It can now be installed in the same way as any other CRAN posted packages.
...
```{r Declaring_Structures_of_Results_ompr}
  ...
  dimnames(results.lambda)<-list(c(LETTERS[1:ND]),c(LETTERS[1:ND]))
  
  results.vweight    <- matrix(rep(-1.0, ND*NX), nrow=ND,ncol=NX)
  dimnames(results.vweight)<-list(c(LETTERS[1:ND]),c("v"))
  
  results.uweight    <- matrix(rep(-1.0, ND*NY), nrow=ND,ncol=NY) 
  dimnames(results.uweight)<-list(c(LETTERS[1:ND]),c("u"))

  results.xslack     <- matrix(rep(-1.0, ND*NX), nrow=ND,ncol=NX) 
  dimnames(results.xslack)<-list(c(LETTERS[1:ND]),c("sx"))

  results.yslack     <- matrix(rep(-1.0, ND*NY), nrow=ND,ncol=NY) 
  dimnames(results.yslack)<-list(LETTERS[1:ND],"sy")
```
...
#print (results.efficiency)
pander(results.efficiency, round=6, caption="Input-oriented Efficiency Scores")
