...
Now that we have covered an introduction to \index{Benchmarking} benchmarking using \index{Data envelopment analysis} data envelopment analysis, let's look at a variety of models applied to sports - in particular, baseball.
...
The range of applications can cover \index{Benchmarking} benchmarking individual players, managers, general managers, and teams. Analyzing individual players is the most common type of baseball application and will be the bulk of this chapter. Each baseball player typically plays a role as a batter, a fielder, and/or a pitcher. The analysis of baseball batters is an updated version of an earlier paper [@Andersonnewmeasurebaseball1997a].

For those readers not interested in baseball, this can be viewed as a set of case studies examining individual performance and organizational performance from a variety of perspectives. It also provides a rich example of data preparation for doing a \index{Benchmarking} benchmarking study.
...
kbl (head(Lahman::Batting), booktabs = T,
       caption="Sample of Lahman Baseball Data") |> 
...
kbl (head(batting), booktabs = T, 
     caption="Limit to Full-time Players")            |>
  kable_styling(latex_options = c("hold_position"))
```
...
kbl(head(cbind(res1920AL$vx, res1920AL$uy)), 
  ...
  add_header_above(c(" " = 1, "Input Weight" = 1, "Output Weights" = 5)) |>
  kable_styling(latex_options = c("hold_position"))
```
...
Another approach is to analyze the data set using the \index{Data envelopment analysis!multiplier model} multiplier model and directly incorporating weight restrictions. For this, we will make use of the `DEAMultiplier`package written by Aurobindh Kalathil Puthanpura.
...
Notice that the scores are up to 1.0 rather than 1.0 and higher as in the \index{TFDEA} TFDEA package. This is because the TFDEA package reports efficiency as $\phi$ whereas the `DEAMultiplier` package reports $\frac{1}{\phi}$. The results are consistent and reciprocals. The value of $\phi$ is consistent with the formulation of the \index{Data envelopment analysis!output-oriented} output-oriented model. The value of $\frac{1}{\phi}$ has the same interpretation $\theta$ where a value less than 1.0 indicates inefficiency. In fact, for the case of constant returns to scale (CRS), the orientation does not make a difference in the value of inefficiency found, in other words, $\theta=\frac{1}{\phi}$ For the sake of this discussion, we will interpret the results as $\frac{1}{\phi}$.
...
$$
 \begin{split}
 \begin{aligned}
    \ & 1 \leq \frac{u_{doubles}} {u_{walks}} \leq \infty \\
...
kbl(head(weights 
         [order(weights[,1], decreasing=TRUE),],20), digits=6, booktabs=T,
    caption="Output Weights",
    col.names=c("Eff", "BB", "X1B", "X2B", "X3B", "HR", "Eff", "BB", "X1B", "X2B", "X3B", "HR")) |>
    add_header_above( c(" " = 1, "Standard Input-Oriented CRS" = 6, "With Weight Restrictions Added" = 6)) |>
 kable_styling(latex_options = c("hold_position", "scale_down")) |>
...
CRSvsCRSWR <- ggplot (collectedeff1, aes(x=as.numeric(CCRIO), y=as.numeric(CCRIOWR))) +
  ...
  geom_text(data=subset(collectedeff1, as.numeric(CCRIO) > 0.999 ), 
            nudge_x = 0.03, aes(x=as.numeric(CCRIO), y=as.numeric(CCRIOWR), 
                                label=nameID))
...
Our \index{Data envelopment analysis} DEA input-output model will be salary used to create regular season wins.
...
```{r, echo=FALSE}
kbl(TRA::poscol(cbind(res1991MLB$eff,res1991MLB$lambda), cutoff=0.00001), 
    booktabs=T, digits=4, 
       caption=
...
moneyballplot2 <- ggplot (
  ...
  geom_point(data=teampayroll2[teampayroll2$teamID %in% 
                                 c("NYA"),],
             aes(x=yearID, y=Efficiency, shape="New York")) + 
  geom_point(data=teampayroll2[teampayroll2$teamID %in% 
                                 c("OAK"),],
             aes(x=yearID, y=Efficiency, shape="Oakland")) +
...
class(teampayroll3$batter)<-"numeric"
class(teampayroll3$catcher)<-"numeric"
class(teampayroll3$firstBase)<-"numeric"
class(teampayroll3$secondBase)<-"numeric"
class(teampayroll3$thirdBase)<-"numeric"
class(teampayroll3$shortstop)<-"numeric"
class(teampayroll3$outfielder)<-"numeric"
...
class(teampayroll3$WinPercent) <- "numeric"

teampayroll3 <- teampayroll3[-1,]

kbl (head(teampayroll3), booktabs=T, digits=2, row.names=F, 
     caption="Team Information") |>
  kable_styling (latex_options = c("hold_position", "scale_down"))
```

```{r}
kbl (head(cbind(teampayroll2$teamID, 
                as.numeric(teampayroll2$Efficiency), 
                as.numeric(teampayroll3$Efficiency))), 
     booktabs=T, digits=4, caption=
         "Sample of BCC-Efficiency vs. Allocative Efficiency",
     col.names=c("Team ID", "VRS", "Allocative")) |>
  kable_styling (latex_options = c("hold_position"))
```
...
-   \index{Malmquist Productivity Index} Malmquist Productivity Index application in baseball
