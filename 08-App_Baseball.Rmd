...
<<<<<<< HEAD
A more generalized data structure for the weight restrictions looks at these as as a series ratios. 
=======
A more generalized data structure for the weight restrictions looks at these as a ratio.  

>>>>>>> 63a7b0dc20a835e43443c9e3c08a9e1ef02c8490
...
collectedeff1 <- data.frame(cbind(res1920ALmult$Efficiency,
                                  ...
                                  res1920ALcross$ce_ave,
                                  res1920ALCEWR$ce_ave) ) 
...
colnames(res1920ALcross$ce_ave)<-"Cross-Efficiency"

pander(cor(cbind(res1920ALmult$Efficiency, res1920ALcross$ce_ave,
...
<<<<<<< HEAD
#collectedeff2$name <- row.names(collectedeff2)
=======
collectedeff2 <- data.frame(cbind(res1920ALmult$Efficiency, 
                                 res1920ALcross$ce_ave,    res1920ALmultWR$Efficiency) )
>>>>>>> 63a7b0dc20a835e43443c9e3c08a9e1ef02c8490
...
=======
ggplot (collectedeff2, aes(x=Efficiency, y=Cross.Efficiency)) +
      xlab("CCR-IO Efficiency") + 
      ylab("Cross-Efficiency") +
      ggtitle("Impact of Cross-Efficiency on 1920 AL CCR-IO Efficiency") +
>>>>>>> 63a7b0dc20a835e43443c9e3c08a9e1ef02c8490
      geom_point(shape=1) +
      geom_abline(slope=1, color="red")+
...
uymean <- as.data.frame(t(colMeans(res1920ALcross$uy)))
vxmean <- as.data.frame(t(colMeans(res1920ALcross$vx)))
rownames(uymean) <- "output weights"
pander (rbind(ybabe, uymean),
        caption = "Ruth's Outputs and Mean Cross-Efficiency Output Weights")
=======
ggplot (collectedeff2, aes(x=Cross.Efficiency, y=Efficiency.w.WR)) +
      xlab("Cross-Efficiency") + 
      ylab("Cross-Efficiency with Weight Restrictions") +
      ggtitle("Impact of Weight Restrictions on 1920 AL CC Efficiency") +
      geom_point(shape=1) +
...
Notably, this calculated value roughly matches the original cross-efficiency score for Babe Ruth of `r res1920ALcross$ce_ave["ruthba01",]`.  *Note that the value should match better than it does!* 

The same procedure could be be used for calculating the cross-efficiency score for each of the other players.  This is not a matter of some (or most) players occassionally violating the relationship in some way but a systematic error.  What this means is that _every_ player is now being assessed against the same fixed and in this case, implausible weighting scheme. If you look carefully at the mean output weights, the mean weight for Home Runs is lower than that of any other output. 

This is in effect a _ganging up_ phenomenon where most players do poorly on Home Runs relative to Babe Ruth so they choose to not put weight on that output.  On the whole, the league then does not on average puts a low weight on Home Runs. Furthermore, outputs that tend to have low values (such as triples) will tend to have higher output weights than those with higher values such as home runs.  Rightly or wrongly, these distributional characteristics of the data set will affect cross-efficiency scores.
